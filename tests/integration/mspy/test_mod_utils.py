...
from mmass.mspy import mod_utils
...
class TestLoad:
    ...
    def test_load_mzdata_extension_real_file(self):
        ...
        test_file = Path(__file__).parent / "../../data/test_small.mzdata"
    ...
    def test_load_mzxml_extension_real_file(self):
        ...
        test_file = Path(__file__).parent / "../../data/test_small.mzXML"
        if not test_file.exists():
            pytest.skip("Test data file not available")
    ...
    def test_load_mzml_extension_real_file(self):
        ...
        test_file = Path(__file__).parent / "../../data/test_small.mzML"
        if not test_file.exists():
            pytest.skip("Test data file not available")
        result = mod_utils.load(test_file, scanID=1)
        assert result is not None
    ...
    def test_load_mgf_extension_real_file(self):
        ...
        test_file = Path(__file__).parent / "../../data/test_tiny.mgf"
        if not test_file.exists():
            pytest.skip("Test data file not available")
        result = mod_utils.load(test_file)
        assert result is not None
    ...
    def test_load_xy_extension_continuous(self):
        ...
        test_file = Path(__file__).parent / "../../data/test_small.xy"
        if not test_file.exists():
            pytest.skip("Test data file not available")
    ...
    def test_load_txt_extension_mocked(self, tmp_path, mocker):
    ...
    def test_load_asc_extension_mocked(self, tmp_path, mocker):
    ...
    def test_load_xml_sniff_mzdata(self, tmp_path, mocker):
    ...
    def test_load_xml_sniff_mzxml(self, tmp_path, mocker):
    ...
    def test_load_xml_sniff_mzml(self, tmp_path, mocker):
    ...
    def test_load_xml_no_marker_raises_valueerror(self, tmp_path):
        """Test load with XML file that has no recognized marker raises ValueError."""
        xml_path = tmp_path / "test.xml"
        with xml_path.open("w") as f:
    ...
    def test_load_unknown_extension_raises_valueerror(self, tmp_path):
        """Test load with unknown file extension raises ValueError."""
        unknown_path = tmp_path / "test.unknown"
        with unknown_path.open("w") as f:
            f.write("dummy content")

        with pytest.raises(ValueError, match="Unknown document type!"):
            mod_utils.load(unknown_path)

    def test_load_extension_case_insensitive_uppercase(self, tmp_path, mocker):
        """Test load handles uppercase extension properly."""
        xy_path = tmp_path / "test.XY"
        with xy_path.open("w") as f:
            f.write("100.0\t1000.0\n")
    ...
    def test_load_mzdata_passes_scanid_to_parser(self, tmp_path, mocker):
        """Test that load passes scanID to parser.scan()."""
        mzdata_path = tmp_path / "test.mzdata"
        with mzdata_path.open("w") as f:
    ...
    def test_load_xy_passes_datatype_to_parser(self, tmp_path, mocker):
    ...
    def test_load_mzxml_parser_receives_scanid(self, tmp_path, mocker):
        """Test that load passes scanID to mzXML parser."""
        mzxml_path = tmp_path / "test.mzxml"
        with mzxml_path.open("w") as f:
            f.write("<mzXML></mzXML>")
    ...
    def test_load_mzml_parser_receives_scanid(self, tmp_path, mocker):
        """Test that load passes scanID to mzML parser."""
        mzml_path = tmp_path / "test.mzml"
        with mzml_path.open("w") as f:
            f.write("<mzML></mzML>")
    ...
    def test_load_returns_scan_object_from_parser(self, tmp_path, mocker):
    ...
    def test_load_parser_returns_false_propagates(self, tmp_path, mocker):
    ...
    def test_load_default_scanid_is_none(self, tmp_path, mocker):
    ...
    def test_load_default_datatype_is_continuous(self, tmp_path, mocker):
    ...
    def test_load_mgf_with_scanid(self, tmp_path, mocker):
...
class TestSave:
    ...
    def test_save_nonempty_data_writes_correct_content(self, tmp_path):
    ...
    def test_save_empty_data_writes_empty_file(self, tmp_path):
        """Test save with empty data list writes empty file."""
        save_path = tmp_path / "empty.txt"
        data = []
    ...
    def test_save_float_format_precision(self, tmp_path):
        """Test that save preserves float precision."""
        save_path = tmp_path / "precision.txt"
    ...
    def test_save_large_numbers(self, tmp_path):
        """Test save with large floating point numbers."""
        save_path = tmp_path / "large.txt"
    ...
    def test_save_zero_values(self, tmp_path):
        """Test save with zero values."""
        save_path = tmp_path / "zeros.txt"
    ...
    def test_save_negative_values(self, tmp_path):
        ...
        save_path = tmp_path / "negative.txt"
    ...
    def test_save_line_count_matches_data_length_property(self, tmp_path):
        ...
        def check_save_line_count(data):
            if not data:
                # Empty data case
                save_path = tmp_path / f"property_test_{id(data)}.txt"
                mod_utils.save(data, save_path)
                content = save_path.read_text()
                assert content == ""
            else:
    ...
    def test_save_overwrites_existing_file(self, tmp_path):
        ...
        save_path = tmp_path / "overwrite.txt"
    ...
    def test_save_with_very_small_numbers(self, tmp_path):
        """Test save with very small numbers."""
        save_path = tmp_path / "small_nums.txt"
