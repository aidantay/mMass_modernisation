...
def simple_scan():
    ...
    profile = np.array([[100.0, 50.0], [101.0, 100.0], [102.0, 75.0], [103.0, 25.0]])
    peaks = [
...
def profile_only_scan():
    ...
    profile = np.array([[100.0, 50.0], [101.0, 100.0], [102.0, 75.0]])
...
class TestScaleAndShift:
    ...
    def test_scaleAndShift_valid_array(self):
        ...
        points = np.array(
            [[100.0, 50.0], [101.0, 100.0], [102.0, 75.0]], dtype=np.float64
        )
...
class TestFilterPoints:
    ...
    def test_filterPoints_valid_array(self):
        ...
        points = np.array(
            [[100.0, 50.0], [100.5, 60.0], [101.0, 100.0]], dtype=np.float64
        )
    ...
    def test_filterPoints_minimal_resolution(self):
        ...
        pts = np.array(
            [[100.0, 50.0], [100.00000001, 100.0], [101.0, 75.0]], dtype=np.float64
        )
...
class TestContainer:
    ...
    def test_container_getBoundingBox_empty(self):
        ...
        assert np.array_equal(bbox[0], np.array([0, 0]))
        assert np.array_equal(bbox[1], np.array([1, 1]))
    ...
    def test_container_getBoundingBox_equal_x_padding(self, simple_scan):
        ...
        profile = np.array([[100.0, 50.0], [100.0, 100.0]])
    ...
    def test_container_getBoundingBox_equal_y_padding(self, simple_scan):
        ...
        profile = np.array([[100.0, 50.0], [101.0, 50.0]])
        spec = Spectrum(Scan(profile=profile))
        cont = Container([spec])
        bbox = cont.getBoundingBox()
        # Check if padding was added (not equal)
    ...
    def test_container_getPoint(self, simple_scan):
        ...
        spec.spectrumScaled = np.array([], dtype=np.float64).reshape(0, 2)
        result = cont.getPoint(0, 100.5)
        assert result is None
    ...
    def test_container_getBoundingBox_advanced(self, simple_scan):
        ...
        class MockObj:
            ...
            def getBoundingBox(self, *args, **kwargs):
                return [np.array([float("inf"), 0]), np.array([1, 1])]
...
class TestAnnotations:
    ...
    def test_annotations_filterPoints(self, simple_annotations_data):
        ...
        assert np.array_equal(annot.points, original)
...
class TestSpectrum:
    ...
    def test_spectrum_getPoint_boundary(self, simple_scan):
        ...
        spec = Spectrum(simple_scan)
        spec.spectrumScaled = np.array([[100.0, 50.0], [101.0, 100.0]])
