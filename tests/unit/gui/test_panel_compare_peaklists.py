...
from mmass.gui import config, images, panel_compare_peaklists
...
def panel(wx_app, mock_parent, setup_images):
    ...
    p = panel_compare_peaklists.PanelComparePeaklists(mock_parent)
    yield p
    if p:
        p.Destroy()
...
def test_init(panel):
    ...
    assert isinstance(panel, panel_compare_peaklists.PanelComparePeaklists)
...
def test_runCompare_force_quit(panel, mocker):
    """Test runCompare handles ForceQuitError."""
    panel.currentPeaklist = mocker.MagicMock()
    panel.currentPeaklist.__iter__.side_effect = mspy.ForceQuitError
