...
from mmass.gui import config, panel_mass_to_formula
...
@pytest.mark.parametrize(
    ...
    [
        (panel_mass_to_formula.ids.ID_massToFormulaSearchPubChem, "PubChem"),
        (panel_mass_to_formula.ids.ID_massToFormulaSearchChemSpider, "ChemSpider"),
        (panel_mass_to_formula.ids.ID_massToFormulaSearchMETLIN, "METLIN"),
        (panel_mass_to_formula.ids.ID_massToFormulaSearchHMDB, "HMDB"),
        (panel_mass_to_formula.ids.ID_massToFormulaSearchLipidMaps, "Lipid MAPS"),
    ],
)
def test_on_item_search(panel, server_id, server_name, mocker):
    ...
    written_content = mock_write_bytes.call_args[0][0]
...
def test_on_item_search_exception(panel, mocker):
    ...
    panel.currentFormulae = [["C6H12O6", 180.0, 181.0, 0.0, None, 1.0, None, None]]
    mock_evt = mocker.Mock()
    mock_evt.GetId.return_value = panel_mass_to_formula.ids.ID_massToFormulaSearchPubChem

    mocker.patch.object(panel.formulaeList, "getSelected", return_value=[0])
    mocker.patch.object(panel.formulaeList, "GetItemData", return_value=0)
    # Mock Path.write_bytes to raise exception
    mocker.patch("pathlib.Path.write_bytes", side_effect=OSError("Test Error"))
...
def test_run_generator(panel, mocker):
    ...
    mocker.patch("mmass.mspy.delta", side_effect=[0.1, 0.001])
...
def test_run_generator_force_quit(panel, mocker):
    """Test runGenerator handles mspy.ForceQuitError (Step 7)."""
    mocker.patch("mmass.mspy.mz", side_effect=mod_stopper.ForceQuitError)
