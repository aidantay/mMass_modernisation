...
from mmass.gui import panel_monomer_library
...
def mock_gui_deps(mocker):
    ...
    return {"mwx": mock_mwx, "images": mock_images}


@pytest.fixture
def panel(wx_app, mock_mspy_monomers, mock_gui_deps):
    parent = wx.Frame(None)
    p = panel_monomer_library.PanelMonomerLibrary(parent)
    yield p
    if p:
        p.Destroy()
    parent.Destroy()
...
def test_makeToolbar_mac(wx_app, mock_gui_deps, mock_mspy_monomers, mocker):
    ...
    evt = wx.PyCommandEvent(
        wx.wxEVT_COMMAND_SEARCHCTRL_CANCEL_BTN, p.search_value.GetId()
    )
