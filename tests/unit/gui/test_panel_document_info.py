...
from mmass.gui import ids, panel_document_info
...
def panel(wx_app, mocker, mock_parent):
    ...
    p = panel_document_info.PanelDocumentInfo(mock_parent)
    yield p
    if p:
        p.Destroy()
...
def test_dlgPresetsName(wx_app, mock_parent, mocker):
    dlg = panel_document_info.DlgPresetsName(mock_parent)
