...
from mmass.gui import dlg_references_editor, ids
...
def editor_dialog(wx_app, mocker):
    ...
    dlg = dlg_references_editor.DlgReferencesEditor(None)
...
def test_dlgGroupName_init(wx_app):
    ...
    dlg = dlg_references_editor.DlgGroupName(None)
    assert dlg.name == ""
    assert dlg.name_value.GetValue() == ""
    dlg.Destroy()

    # Test with provided name
    dlg = dlg_references_editor.DlgGroupName(None, name="MyGroup")
...
def test_dlgSelectItemsToImport_init_and_list(wx_app):
    ...
    items = {"Group 1": [("I1", 1.0)], "Group 2": [("I2", 2.0), ("I3", 3.0)]}
    dlg = dlg_references_editor.DlgSelectItemsToImport(None, items)
...
def test_editor_init(editor_dialog):
    ...
    dlg, _mock_refs, _ = editor_dialog
    ...
    choices = [
        dlg.groupName_choice.GetString(i)
        for i in range(dlg.groupName_choice.GetCount())
    ]
...
def test_editor_add_group(editor_dialog, mocker):
    ...
    dlg, mock_refs, _mock_compounds = editor_dialog
...
"""
    ...
    assert not dlg.readLibraryXML("dummy_path")
...
def test_editor_import_workflow(editor_dialog, mocker):
    ...
    MockMsg.return_value.ShowModal.return_value = ids.ID_dlgReplace
...
def test_editor_import_replace_all(editor_dialog, mocker):
    ...
    MockMsg.return_value.ShowModal.return_value = ids.ID_dlgReplaceAll
...
def test_editor_import_skip(editor_dialog, mocker):
    ...
    dlg, mock_refs, _ = editor_dialog
    imported_data = {"Group A": [("New Item A", 500.0)]}
    ...
    MockMsg.return_value.ShowModal.return_value = ids.ID_dlgSkip
