...
from mmass.gui import config, images, panel_envelope_fit
...
def mock_config(mocker):
    ...
    mocker.patch.dict(
        ...
        clear=False,
    )  # Don't clear processing as it has sub-dicts
    return config
...
def panel(wx_app, mock_parent, mock_config, mock_images, mocker):
    ...
    p = panel_envelope_fit.PanelEnvelopeFit(mock_parent)
    yield p
    if p:
        p.Destroy()
...
def test_run_envelope_fit_force_quit(panel, mock_config, mocker):
    """Verify runEnvelopeFit handling mspy.ForceQuitError."""
    panel.currentCompound = mocker.Mock()
    panel.currentCompound.formula.return_value = "C6H12O6"

    mocker.patch("mmass.mspy.EnvFit", side_effect=mod_stopper.ForceQuitError)
    panel.runEnvelopeFit()
