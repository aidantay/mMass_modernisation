...
from mmass.gui import panel_compounds_search
...
def mock_gui_env(mocker):
    ...
    # Setup mock_mspy
    class MockForceQuitError(Exception):
        pass

    mock_mspy = mocks["mmass.mspy"]
    mock_mspy.ForceQuitError = MockForceQuitError
...
def test_onToolSelected(panel, mock_gui_env, mocker):
    ...
    mocker.patch.object(panel_compounds_search.ids, "ID_compoundsSearchFormula", 1002)
    mocker.patch.object(panel_compounds_search.ids, "ID_compoundsSearchCompounds", 1001)
...
def test_runGenerateIons(panel, mock_gui_env, mocker):
    ...
    assert panel.currentCompounds[0][3] is None
...
def test_runGenerateIons_force_quit(panel, mock_gui_env, mocker):
    ...
    # ForceQuitError exception
    mock_mspy.CHECK_FORCE_QUIT.side_effect = mock_mspy.ForceQuitError()
...
def test_setData(panel, mocker):
    ...
    assert panel.currentCompounds[0][5] is None
...
def test_onListFilter(panel, mock_gui_env, mocker):
    mocker.patch.object(panel_compounds_search.ids, "ID_listViewMatched", 1004)
    mocker.patch.object(panel_compounds_search.ids, "ID_listViewUnmatched", 1005)
    mocker.patch.object(panel_compounds_search.ids, "ID_listViewAll", 1003)
