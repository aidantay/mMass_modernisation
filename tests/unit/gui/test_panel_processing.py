...
from mmass.gui import config, doc, images, libs, panel_processing
...
def panel(mock_parent, mock_config, mock_images, mocker):
    ...
    p = panel_processing.PanelProcessing(mock_parent)
    yield p
    if p:
        p.Destroy()
...
def test_onToolSelected(panel, mock_images, mocker):
    ...
    for id_name, tool_str in tools:
        real_id = getattr(panel_processing.ids, id_name)
...
def test_setData(panel, mocker):
    ...
    doc_obj.spectrum.profile = np.array([[100, 10], [200, 20]])
...
def test_dlgPresetsName(wx_app, mock_parent, mocker):
    ...
    dlg = panel_processing.DlgPresetsName(mock_parent)
...
def test_onBaselineChanged(panel, mocker):
    ...
    doc_obj.spectrum.baseline.return_value = np.array([[100, 5], [200, 5]])
...
def test_onPreview_baseline(panel, mocker, sync_thread):
    ...
    mocker.patch("mmass.mspy.subbase", return_value=np.array([[100, 5], [200, 15]]))
...
def test_onPreview_smoothing(panel, mocker, sync_thread):
    ...
    mocker.patch("mmass.mspy.smooth", return_value=np.array([[100, 11], [200, 19]]))
...
def test_onPreview_math(panel, mocker, sync_thread):
    ...
    mocker.patch(
        "mmass.mspy.normalize", return_value=np.array([[100, 0.1], [200, 0.2]])
    )
    mocker.patch(
        "mmass.mspy.multiply", return_value=np.array([[100, 10], [200, 20]])
    )
...
def test_onApply_peakpicking(panel, mocker, sync_thread):
    ...
    doc_obj = mocker.MagicMock()
    doc_obj.spectrum.hasprofile.return_value = True
    doc_obj.spectrum.profile = np.array([[100, 10], [200, 20]], dtype=np.float64)
    doc_obj.annotations = []
    doc_obj.sequences = []
    panel.currentDocument = doc_obj
...
def test_runApplyMath_forcequit(panel, mocker):
    """Test ForceQuitError exception in runApplyMath."""
    panel.onToolSelected(tool="math")
    panel.mathOperationNorm_radio.SetValue(True)
    panel.currentDocument = mocker.Mock()

    # Mock backup to raise ForceQuitError
    panel.currentDocument.backup.side_effect = mod_stopper.ForceQuitError()

    # runApplyMath should catch ForceQuitError and return
    panel.runApplyMath()

    # Verify no bell on cancel
    # Actually runApplyMath doesn't ring bell on ForceQuitError
...
def test_checkIsotopeMassTolerance(panel, mocker):
    ...
    dlg_mock = mocker.patch("mmass.gui.mwx.DlgMessage")
