...
def mock_dependencies(wx_app, mocker):
    ...
    mock_plot.Points = mocker.Mock(
        side_effect=lambda *args, **kwargs: mocker.Mock(
            setProperties=mocker.Mock(), setNormalization=mocker.Mock()
        )
    )
    mock_plot.Spectrum = mocker.Mock(
        side_effect=lambda *args, **kwargs: mocker.Mock(
            setProperties=mocker.Mock(), setNormalization=mocker.Mock()
        )
    )
    mock_plot.Annotations = mocker.Mock(
        side_effect=lambda *args, **kwargs: mocker.Mock(
            setProperties=mocker.Mock(), setNormalization=mocker.Mock()
        )
    )
...
def test_dlgSpectrumOffset(wx_app, mocker):
    ...
    mocker.patch.object(
        panel_spectrum.DlgSpectrumOffset, "makeGUI", return_value=mock_sizer
    )
    ...
    mocker.patch.object(
        panel_spectrum.DlgSpectrumOffset, "GetSize", return_value=(100, 100)
    )
...
def test_dlgCanvasProperties(wx_app, mocker):
    ...
    mock_sizer = mocker.Mock()
    mocker.patch.object(
        panel_spectrum.DlgCanvasProperties, "makeGUI", return_value=mock_sizer
    )
    ...
    mocker.patch.object(
        panel_spectrum.DlgCanvasProperties, "GetSize", return_value=(100, 100)
    )
...
def test_makeSpectrumCanvas(panel):
    ...
    all_keys = []
    for call in calls:
        all_keys.extend(list(call[1].keys()))
...
def test_makeToolbar(panel):
    ...
    assert panel.showLabels_butt.GetId() == ids.ID_viewLabels
...
def test_updateCanvasProperties(panel, mock_documents, mocker):
    ...
    mocker.patch.object(mod.ids, "ID_viewLabels", 151)
...
def test_labelEnvelope(panel, mock_documents, mocker):
    ...
    peaks = [
        mocker.Mock(mz=100.0, ai=500.0, base=0.0, intensity=500.0, sn=10.0),
...
def test_updateCanvasProperties_all_ids(panel, id_name, config_key, config_obj, mocker):
    ...
    mocker.patch.object(mod.ids, id_name, 999)
...
@pytest.mark.parametrize(
    "tool",
    ["ruler", "labelpeak", "labelpoint", "labelenvelope", "deletelabel", "offset"],
)
