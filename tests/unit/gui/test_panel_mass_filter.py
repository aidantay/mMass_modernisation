...
from mmass.gui import config, ids, images, libs, panel_mass_filter
...
def panel(wx_app, mock_parent, mocker):
    ...
    mocker.patch.dict(
        config.main,
        {"mzDigits": 4, "ppmDigits": 2, "macListCtrlGeneric": 0, "reverseScrolling": 0},
    )
    ...
    mock_refs = {"Test Group": [("Ref 1", 100.0), ("Ref 2", 200.0)]}
...
def test_onListFilter(panel, mocker):
    ...
    event.GetId.return_value = ids.ID_listViewMatched
    panel.onListFilter(event)
    assert panel._referencesFilter == 1

    # Unmatched only
    event.GetId.return_value = ids.ID_listViewUnmatched
    panel.onListFilter(event)
    assert panel._referencesFilter == -1

    # All
    event.GetId.return_value = ids.ID_listViewAll
    panel.onListFilter(event)
    assert panel._referencesFilter == 0


def test_onItemSelected(panel, mock_parent, mocker):
    panel.currentReferences = [["Ref 1", 100.0, None, True, []]]
    event = mocker.MagicMock()
...
def test_onAnnotate_happy_path(panel, mock_document, mock_parent, mocker):
    ...
    panel.currentReferences = [["Ref 1", 100.0, 0.01, True, [mock_ann]]]
...
def test_clearMatches(panel, mocker):
    panel.currentReferences = [["Ref 1", 100.0, 0.01, True, [mocker.MagicMock()]]]
...
def test_onListRMU(panel, mocker):
    ...
    mock_menu_inst.Append.assert_any_call(ids.ID_listViewAll, "Show All", "", wx.ITEM_RADIO)
