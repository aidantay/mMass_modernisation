...
from mmass.gui import dlg_compounds_editor, ids, libs
...
def dlg_editor_instance(wx_app, compounds_lib, mocker):
    ...
    dlg = dlg_compounds_editor.DlgCompoundsEditor(parent)
    yield dlg
    dlg.Destroy()
    parent.Destroy()
...
def test_dlgGroupName_init(wx_app):
    ...
    dlg = dlg_compounds_editor.DlgGroupName(parent)
    assert dlg.name == ""
    assert dlg.name_value.GetValue() == ""
    dlg.Destroy()

    # Test with initial name
    dlg = dlg_compounds_editor.DlgGroupName(parent, name="TestGroup")
...
def test_dlgSelectItemsToImport_init(wx_app, mocker):
    ...
    dlg = dlg_compounds_editor.DlgSelectItemsToImport(parent, items)
...
def test_dlgCompoundsEditor_readLibraryXML(dlg_editor_instance, tmp_path):
...
def test_dlgCompoundsEditor_onImport_replace_logic(
    ...
    # Test 'Replace'
    mock_msg = mocker.patch("mmass.gui.mwx.DlgMessage")
    mock_msg.return_value.ShowModal.return_value = ids.ID_dlgReplace
    dlg_editor_instance.onImport(None)
    assert "new" in compounds_lib["Group1"]

    # Test 'Skip'
    compounds_lib["Group1"] = {"old": old_compound}
    mock_msg.return_value.ShowModal.return_value = ids.ID_dlgSkip
    dlg_editor_instance.onImport(None)
...
def test_dlgCompoundsEditor_readLibraryXML_errors(dlg_editor_instance, tmp_path):
    """Test error handling in XML reading."""
    # Case: File does not exist
    assert dlg_editor_instance.readLibraryXML(pathlib.Path("non_existent.xml")) is False
