...
from mmass.gui import config, ids, images, panel_document_export
...
def export_panel(wx_app, mock_parent, mock_images, monkeypatch_config, mocker):
    ...
    panel = panel_document_export.PanelDocumentExport(mock_parent)
...
def test_init_specific_tool(mock_parent, mock_images, monkeypatch_config, mocker):
    ...
    # Test peaklist
    panel = panel_document_export.PanelDocumentExport(mock_parent, tool="peaklist")
    ...
    # Test spectrum
    panel = panel_document_export.PanelDocumentExport(mock_parent, tool="spectrum")
...
def test_onToolSelected(export_panel, mocker):
    ...
    event = wx.CommandEvent(wx.EVT_BUTTON.typeId, ids.ID_documentExportPeaklist)
    ...
    event = wx.CommandEvent(wx.EVT_BUTTON.typeId, ids.ID_documentExportSpectrum)
    export_panel.onToolSelected(event)
    ...
    event = wx.CommandEvent(wx.EVT_BUTTON.typeId, ids.ID_documentExportImage)
    export_panel.onToolSelected(event)
    assert export_panel.currentTool == "image"
    assert export_panel.GetTitle() == "Export Spectrum Image"
    assert export_panel.mainSizer.IsShown(1)
...
def test_init_invalid_resolution(
    wx_app, mock_parent, mock_images, monkeypatch_config, mocker
):
...
def test_onExportImage_success(export_panel, mocker):
    ...
    path = str(pathlib.Path("/tmp") / "test_spectrum.png")
    ...
    mock_run.assert_called_once_with(path)
...
def test_onExportPeaklist_success(export_panel, mocker):
    ...
    instance = mock_file_dlg.return_value
    instance.ShowModal.return_value = wx.ID_OK
    path = str(pathlib.Path("/tmp") / "test_peaklist.txt")
    instance.GetPath.return_value = path
    ...
    assert mock_thread.call_args[1]["kwargs"] == {"path": path}
...
def test_onExportSpectrum_success(export_panel, mocker):
    ...
    instance = mock_file_dlg.return_value
    instance.ShowModal.return_value = wx.ID_OK
    path = str(pathlib.Path("/tmp") / "test_spectrum.txt")
    instance.GetPath.return_value = path
...
def test_runExportImage_units_conversion(export_panel, mock_parent, mocker):
    ...
    mock_get_bitmap = mocker.patch.object(
        mock_parent, "getSpectrumBitmap", return_value=mock_bitmap
    )
    ...
    mock_get_bitmap.assert_called_with(
        pytest.approx(393.7), pytest.approx(787.4), {"drawings": 2, "fonts": 3}
    )
...
def test_runExportPeaklist_ascii_no_headers(
    export_panel, mock_parent, tmp_path, mocker
):
...
def test_runExportSpectrum_success(export_panel, mock_parent, tmp_path, mocker):
    ...
    mock_get_points = mocker.patch.object(
        mock_parent, "getCurrentSpectrumPoints", return_value=spectrum_data
    )
    ...
    assert pathlib.Path(path_tab).exists()
    with pathlib.Path(path_tab).open() as f:
        content = f.read()
    assert content == "100.000000\t500.000000\n101.000000\t600.000000\n"
