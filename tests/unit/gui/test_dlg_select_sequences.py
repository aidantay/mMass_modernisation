...
from mmass.gui import dlg_select_sequences
...
def dlg(wx_app, mock_sequences, mocker):
    ...
    dialog = dlg_select_sequences.DlgSelectSequences(parent, mock_sequences)
    yield dialog
    dialog.Destroy()
    parent.Destroy()
...
def test_updateSequenceList_empty(wx_app, mocker):
    ...
    parent = wx.Frame(None)
    dlg = dlg_select_sequences.DlgSelectSequences(parent, [])
