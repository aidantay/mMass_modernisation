...
from mmass.gui import ids, panel_documents
...
def test_fileDropTarget(mocker):
    ...
    target = panel_documents.FileDropTarget(mock_fn)
...
def test_documentsTree_core_methods(wx_app, mocker):
    ...
    frame = wx.Frame(None)
    tree = panel_documents.DocumentsTree(frame, -1)
...
def test_panelDocuments_initialization(wx_app, mocker):
    ...
    panel = panel_documents.PanelDocuments(parent, documents)

    assert panel.parent == parent
    assert panel.documents == documents
    assert hasattr(panel, "documentTree")
    assert isinstance(panel.documentTree, panel_documents.DocumentsTree)
...
def test_panelDocuments_onRMU(wx_app, mocker):
    ...
    assert ids.ID_sequenceNew in appended_ids
    assert ids.ID_documentFlip in appended_ids
    assert ids.ID_documentClose in appended_ids
    ...
    assert ids.ID_sequenceDigest in appended_ids
    assert ids.ID_sequenceSearch in appended_ids
    ...
    assert ids.ID_documentAnnotationEdit in appended_ids
    assert ids.ID_documentAnnotationDelete in appended_ids
