...
from mmass.gui import config, panel_prospector
...
@pytest.fixture
def mock_config():
    original_prospector = copy.deepcopy(config.prospector)
    yield config.prospector
    config.prospector.clear()
    config.prospector.update(original_prospector)
...
def test_init(wx_app, mock_parent, mock_config):
    ...
    panel = panel_prospector.PanelProspector(mock_parent)
    ...
    panel = panel_prospector.PanelProspector(mock_parent, tool="mstag")
...
def test_on_tool_selected(wx_app, mock_parent, mock_config, mocker):
    ...
    mocker.patch.object(panel_prospector.ids, "ID_prospectorMSFit", 1001)
    mocker.patch.object(panel_prospector.ids, "ID_prospectorMSTag", 1002)
    mocker.patch.object(panel_prospector.ids, "ID_prospectorQuery", 1003)
    ...
    assert config.prospector["common"]["searchType"] == "mstag"
    ...
    assert panel.GetTitle() == "Protein Prospector - MS-Fit"
    assert panel.search_butt.IsEnabled()
    assert config.prospector["common"]["searchType"] == "msfit"
...
def test_check_params(wx_app, mock_parent, mock_config, mocker):
    ...
    panel.onToolSelected(tool="msfit")
    config.prospector["msfit"]["taxonomy"] = "All"
    config.prospector["msfit"]["database"] = "SwissProt"
    config.prospector["msfit"]["enzyme"] = "Trypsin"
    config.prospector["msfit"]["instrument"] = "ESI-Q-TOF"
    config.prospector["msfit"]["peptideTol"] = 0.1
    config.prospector["msfit"]["variableMods"] = ["Mod1"]
...
def test_make_search_html(wx_app, mock_parent, mock_config):
    ...
    # MS-Fit
    panel.onToolSelected(tool="msfit")
    config.prospector["msfit"]["title"] = "MSFit & Title"
    config.prospector["msfit"]["massType"] = "Monoisotopic"
    config.prospector["msfit"]["proteinMassLow"] = 10.0
    config.prospector["msfit"]["proteinMassHigh"] = 200.0
    config.prospector["msfit"]["fixedMods"] = ["Fixed1"]
    config.prospector["msfit"]["variableMods"] = ["Var1"]
    config.prospector["msfit"]["database"] = "SwissProt"
    config.prospector["msfit"]["taxonomy"] = "All"
    config.prospector["msfit"]["enzyme"] = "Trypsin"
    config.prospector["msfit"]["miscleavages"] = "1"
    config.prospector["msfit"]["instrument"] = "MALDI-TOFTOF"
    config.prospector["msfit"]["pfactor"] = "0.4"
    config.prospector["msfit"]["minMatches"] = "4"
    config.prospector["msfit"]["peptideTol"] = "0.1"
    config.prospector["msfit"]["peptideTolUnits"] = "Da"
    config.prospector["msfit"]["maxMods"] = "1"
    config.prospector["msfit"]["report"] = "5"
...
def test_on_search(wx_app, mock_parent, mock_config, mocker, tmp_path):
    ...
    mock_file_path = mock_path.return_value.__truediv__.return_value
    mock_write_text = mock_file_path.write_text
    mock_as_uri = mock_file_path.as_uri
    mock_as_uri.return_value = "file:///tmp/mmass_prospector_search.html"

    mock_open_browser = mocker.patch("webbrowser.open")

    panel.onSearch(None)

    mock_write_text.assert_called_with("<html>Test</html>", encoding="utf-8")
...
def test_on_close(wx_app, mock_parent, mock_config, mocker, tmp_path):
    ...
    mock_path = mocker.patch("pathlib.Path")
    mock_file_path = mock_path.return_value.__truediv__.return_value
    mock_unlink = mock_file_path.unlink
    mock_destroy = mocker.patch.object(panel, "Destroy")

    panel.onClose(None)

    mock_path.assert_called_with(str(tmp_path))
...
def test_update_form(wx_app, mock_parent, mock_config):
    ...
    config.prospector["msfit"]["database"] = "NCBInr"
    config.prospector["msfit"]["taxonomy"] = "HUMAN MOUSE"
    config.prospector["msfit"]["enzyme"] = "Trypsin"
    config.prospector["msfit"]["instrument"] = "ESI-Q-TOF"
    config.prospector["msfit"]["fixedMods"] = ["Carbamidomethyl (C)"]
    config.prospector["msfit"]["variableMods"] = ["Oxidation (M)"]
