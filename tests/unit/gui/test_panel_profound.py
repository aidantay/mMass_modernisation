...
from mmass.gui import config, images, panel_profound
...
def profound_panel(wx_app, mock_parent, mocker):
    ...
    panel = panel_profound.PanelProfound(mock_parent)
    yield panel
    if panel:
...
def test_initialization(profound_panel):
    ...
    assert isinstance(profound_panel, panel_profound.PanelProfound)
...
def test_onClose(profound_panel, mocker, tmp_path):
...
def test_onToolSelected(profound_panel, mocker):
    ...
    mocker.patch.object(panel_profound.ids, "ID_profoundQuery", 1001)
    mocker.patch.object(panel_profound.ids, "ID_profoundPMF", 1002)
...
def test_onModificationSelected(profound_panel):
    ...
    assert (
        ...
        in profound_panel.paramVariableMods_label.GetLabel()
    )
...
def test_onSearch_success(profound_panel, mocker, tmp_path):
    ...
    mock_as_uri.return_value = "file:///tmp/mmass_profound_search.html"

    mock_open_browser = mocker.patch("webbrowser.open")

    profound_panel.onSearch(None)

    mock_write_text.assert_called_with("<html></html>", encoding="utf-8")
    mock_open_browser.assert_called_with(
        "file:///tmp/mmass_profound_search.html", autoraise=1
    )


def test_onSearch_failure(profound_panel, mocker, tmp_path):
    ...
    mock_path = mocker.patch("pathlib.Path")
    mock_file_path = mock_path.return_value.__truediv__.return_value
    mock_file_path.write_text.side_effect = OSError("Write error")
