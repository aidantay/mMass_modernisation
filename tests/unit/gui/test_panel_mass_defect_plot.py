...
from mmass.gui import config, panel_mass_defect_plot
...
def test_initialization(panel):
    ...
    assert isinstance(panel, panel_mass_defect_plot.PanelMassDefectPlot)
    ...
    # kendrickFormula
    assert (
        panel.kendrickFormula_value.GetValue()
    ...
    # checkboxes
    assert panel.removeIsotopes_check.GetValue() == bool(
        config.massDefectPlot["removeIsotopes"]
    )
    assert panel.ignoreCharge_check.GetValue() == bool(
        config.massDefectPlot["ignoreCharge"]
    )
    assert panel.showNotations_check.GetValue() == bool(
...
def test_make_data_points(panel, mocker):
    ...
    mocked_calc = mocker.patch.object(
        panel, "calcDataPoints", return_value=[(1, 2), (3, 4)]
    )
...
def test_on_plot_canvas_lmu_no_doc(panel, mocker):
    ...
    mock_evt.Skip.assert_called_once()
    assert (
        not hasattr(panel.parent, "updateMassPoints")
        or not panel.parent.updateMassPoints.called
    )
...
def test_update_plot_canvas_labels(panel, mocker):
    ...
    calls = [
        mocker.call(xLabel="nominal mass"),
        mocker.call(yLabel="relative mass defect"),
    ]
    ...
    panel.updatePlotCanvasLabels()
    calls = [
        mocker.call(xLabel="kendrick mass"),
        mocker.call(yLabel="kendrick mass defect"),
    ]
    mocked_set.assert_has_calls(calls, any_order=True)
