...
def test_appInit_mac(mocker):
    ...
    try:
        ...
        wx.SystemOptions.SetOptionInt.assert_called_with(
            "mac.listctrl.always_use_generic", True
        )
...
def test_validator(mocker, frame):
    ...
    # Test Transfer
    assert val.TransferToWindow()
    assert val.TransferFromWindow()
