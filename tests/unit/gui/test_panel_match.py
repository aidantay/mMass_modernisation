...
from mmass.gui import config, doc, ids, images, panel_match
from mmass.mspy import mod_stopper, obj_peak, obj_peaklist
...
def test_onToolSelected(panel, mocker):
    ...
    mock_event.GetId.return_value = ids.ID_matchSummary
    ...
    mock_event.GetId.return_value = ids.ID_matchErrors
    panel.onToolSelected(evt=mock_event)
...
def test_onMatch(panel, mocker):
    ...
    mock_thread_instance.is_alive.return_value = (
        False  # Stop the while loop immediately
    )
...
def test_runMatch_massfilter(panel, mocker):
    ...
    peak = obj_peak.Peak(mz=100.1, ai=1000)
...
def test_runMatch_force_quit(panel, mocker):
    """Test runMatch when mspy.CHECK_FORCE_QUIT raises ForceQuitError."""
    panel.currentModule = "massfilter"
    panel.currentData = [["label", 100.0, 0.1, [mocker.Mock()]]]
    panel.currentPeaklist = [obj_peak.Peak(mz=100.1, ai=1000)]

    mocker.patch("mmass.mspy.CHECK_FORCE_QUIT", side_effect=mod_stopper.ForceQuitError)
    panel.runMatch()
...
def test_makeCurrentPeaklist(panel):
    ...
    p1 = obj_peak.Peak(mz=100.0, ai=1000)
    p2 = obj_peak.Peak(mz=200.0, ai=500)
    panel.currentPeaklist = obj_peaklist.Peaklist([p1, p2])
    ...
    assert isinstance(res, obj_peaklist.Peaklist)
    assert len(res) == 2

    mz100 = next(p for p in res if p.mz == 100.0)
    mz200 = next(p for p in res if p.mz == 200.0)
