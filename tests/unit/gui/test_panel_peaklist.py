...
from mmass.gui import panel_peaklist
...
def mock_config(mocker):
    ...
    mock_conf.export = {"peaklistColumns": ["mz", "ai"]}
    return mock_conf
...
def test_fileDropTarget_onDropFiles(mocker):
    ...
    # Instantiate panel_peaklist.
    target = panel_peaklist.FileDropTarget(mock_callback)
...
def test_dlgThreshold_init(real_parent):
    dlg = panel_peaklist.DlgThreshold(real_parent)
...
def test_dlgCopy_init(real_parent, mock_config):
    dlg = panel_peaklist.DlgCopy(real_parent)
...
def test_panelPeaklist_init(real_parent, mock_config, mock_images):
    panel = panel_peaklist.PanelPeaklist(real_parent)
...
def test_panelPeaklist_setData(
    real_parent, mock_config, mock_images, mock_document, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
...
def test_panelPeaklist_updatePeakList(real_parent, mock_config, mock_images, mocker):
    ...
    mock_config.main["peaklistColumns"] = [
        "mz",
        "ai",
        "int",
        "base",
...
def test_panelPeaklist_updatePeakListItem_edge_cases(
...
def test_panelPeaklist_onListKey_selectAll(
    real_parent, mock_config, mock_images, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
    ...
    for x in range(3):
        mock_set_item_state.assert_any_call(
...
def test_panelPeaklist_onDeleteSelected(real_parent, mock_config, mock_images, mocker):
    ...
    mocker.patch.object(
        panel.peakList, "GetItemData", side_effect=[10, 12]
    )  # Indices in peaklist
...
def test_panelPeaklist_onDeleteByThreshold(
    real_parent, mock_config, mock_images, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
    ...
    peak1.mz = 100.0
    peak1.ai = 1000.0
    peak1.intensity = 1100.0
    peak1.ri = 0.1
    peak1.sn = 5.0
    peak2 = mocker.Mock()
    peak2.mz = 200.0
    peak2.ai = 2000.0
    peak2.intensity = 2100.0
    peak2.ri = 0.5
    peak2.sn = 15.0
    peak3 = mocker.Mock()
    peak3.mz = 300.0
    peak3.ai = 3000.0
    peak3.intensity = 3100.0
    peak3.ri = 1.0
    peak3.sn = 25.0
...
def test_panelPeaklist_getPeakEditorData_valid(
    real_parent, mock_config, mock_images, mock_mspy
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
...
def test_panelPeaklist_getPeakEditorData_invalid(
    real_parent, mock_config, mock_images, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
    mock_bell = mocker.patch("mmass.gui.panel_peaklist.wx.Bell")
...
def test_panelPeaklist_onAnnotate(
    real_parent, mock_config, mock_images, mock_doc, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
    ...
    mock_doc.Annotation.assert_called_once_with(
...
def test_panelPeaklist_onSendToMassToFormula(
    real_parent, mock_config, mock_images, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
...
def test_panelPeaklist_copyToClipboard(real_parent, mock_config, mock_images, mocker):
    ...
    peak.ai = 1000.0
    peak.base = 0.0
    peak.intensity = 1100.0
    peak.ri = 0.5
    peak.sn = 10.0
    peak.charge = 1
    peak.mass.return_value = 124.456
    peak.fwhm = 0.1
    peak.resolution = 1234.0
    peak.group = "G1"
    ...
    mock_config.export["peaklistColumns"] = [
        "mz",
        "ai",
        "base",
        "int",
    ...
    expected_buff = (
        "123.456\t1000.0\t0.0\t1100.0\t50.0\t10.0\t1\t124.456\t0.1\t1234.0\tG1"
    )
...
def test_panelPeaklist_onAddPeak_invalid_data(
    real_parent, mock_config, mock_images, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
    panel.currentDocument = mocker.Mock()
...
def test_panelPeaklist_onReplacePeak_invalid_data(
    real_parent, mock_config, mock_images, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
    panel.currentDocument = mocker.Mock()
...
def test_panelPeaklist_onAnnotate_cancel(
    real_parent, mock_config, mock_images, mock_doc, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
    mock_document = mocker.Mock()
...
def test_panelPeaklist_onDeleteByThreshold_cancel(
    real_parent, mock_config, mock_images, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
    panel.currentDocument = mocker.Mock()
...
def test_panelPeaklist_copyToClipboard_cancel(
    real_parent, mock_config, mock_images, mocker
):
    panel = panel_peaklist.PanelPeaklist(real_parent)
    mocker.patch.object(panel.peakList, "getSelected", return_value=[0])
