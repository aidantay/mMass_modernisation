...
from mmass.gui import config, ids, images, libs, panel_calibration
from mmass.mspy import mod_basics, mod_calibration, obj_peak, obj_peaklist
...
def mock_mspy_dependencies(monkeypatch, mocker):
    ...
    monkeypatch.setattr(mod_calibration, "calibration", mock_calib)
...
def mock_config_and_libs(monkeypatch):
    ...
    monkeypatch.setattr(
        config,
        "calibration",
        {"fitting": "linear", "units": "Da", "tolerance": 0.1, "statCutOff": 100.0},
    )
    ...
    monkeypatch.setattr(config, "spectrum", {"axisFontSize": 10})

    # Mock libs.references
    mock_refs = {"Test List": [("ref1", 100.0), ("ref2", 200.0)]}
...
def mock_document(mocker):
    ...
    p1 = obj_peak.Peak(mz=100.0, ai=1000.0)
    p2 = obj_peak.Peak(mz=200.0, ai=500.0)
    peaklist = obj_peaklist.Peaklist([p1, p2])
...
@pytest.fixture
def panel(
    wx_app, mock_main_frame, mock_mspy_dependencies, mock_config_and_libs, mock_images
):
    """Fixture to instantiate gui.panel_calibration.PanelCalibration."""
    p = panel_calibration.PanelCalibration(mock_main_frame)
    yield p
    if p:
        p.Destroy()
...
def test_panel_initialization(panel):
    ...
    assert isinstance(panel, panel_calibration.PanelCalibration)
...
def test_tool_switching(panel):
    ...
    event = wx.CommandEvent(wx.EVT_BUTTON.typeId, ids.ID_calibrationErrors)
    ...
    event = wx.CommandEvent(wx.EVT_BUTTON.typeId, ids.ID_calibrationReferences)
