...
def libs(module_mocker):
    ...
    mock_config.confdir = Path("/tmp/mmass_test_libs_config")
...
def test_load_functions_missing_file(clean_libs, libs):
    ...
    # They use xml.dom.minidom.parse which raises FileNotFoundError if file missing
...
def test_darwin_initialization(tmpdir, mocker):
    ...
    with (Path(configs_dir) / "presets.xml").open("w") as f:
        f.write("<presets/>")
...
def test_darwin_copy(tmpdir, mocker):
    ...
    conf_dir = Path(tmpdir.mkdir("conf_darwin"))
    configs_dir = str(tmpdir.mkdir("configs"))
    ...
    for item in (
        ...
        with (Path(configs_dir) / item).open("w") as f:
            f.write("<test/>")
    ...
    # Re-import
    import mmass.gui.libs as libs_reloaded
