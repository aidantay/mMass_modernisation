...
from mmass.gui import config, images, libs, panel_mascot
...
def mascot_panel(mocker, wx_app, mock_parent):
    ...
    panel = panel_mascot.PanelMascot(mock_parent)
    yield panel
    if panel:
...
def test_initialization(mascot_panel):
    ...
    assert isinstance(mascot_panel, panel_mascot.PanelMascot)
...
def test_onToolSelected(mocker, mascot_panel):
    ...
    tools = [
        (panel_mascot.ids.ID_mascotPMF, "pmf", 1),
        (panel_mascot.ids.ID_mascotMIS, "mis", 2),
        (panel_mascot.ids.ID_mascotSQ, "sq", 3),
        (panel_mascot.ids.ID_mascotQuery, "query", 4),
    ]
...
def test_onProcessing(mocker, mascot_panel):
    ...
    mocker.patch.object(mascot_panel, "Layout")
    # Start processing
...
def test_checkParams_errors(mocker, mascot_panel):
    ...
    _args, kwargs = mock_dlg.call_args
...
def test_getServerParams_success(mocker, mascot_panel):
    ...
    panel_mascot.config.mascot["common"]["server"] = "TestServer"
    panel_mascot.libs.mascot["TestServer"] = {
        "host": "localhost",
        "path": "/mascot/",
        "params": "params.txt",
    }
...
def test_getServerParams_failure(mocker, mascot_panel):
    ...
    panel_mascot.config.mascot["common"]["server"] = "TestServer"
    panel_mascot.libs.mascot["TestServer"] = {"host": "localhost", "path": "/", "params": ""}
...
def test_makeSearchHTML(mocker, mascot_panel):
    ...
    panel_mascot.config.mascot["common"]["server"] = "Matrix Science"
    panel_mascot.libs.mascot["Matrix Science"] = {
        "protocol": "http",
...
def test_onClose_deletes_temp_file(mocker, mascot_panel, tmp_path):
