...
from mmass.gui import config, mwx, panel_sequence
from mmass.mspy import blocks, mod_proteo, obj_sequence
...
def panel(wx_app, mocker):
    ...
    if "A" not in blocks.monomers:
        blocks.monomers["A"] = blocks.Monomer(
            abbr="A", name="Alanine", formula="C3H5NO", category="_InternalAA"
        )
    if "M" not in blocks.monomers:
        blocks.monomers["M"] = blocks.Monomer(
            abbr="M", name="Methionine", formula="C5H9NOS", category="_InternalAA"
        )

    if "Oxidation" not in blocks.modifications:
        mod = blocks.Modification(
            name="Oxidation", gainFormula="O", lossFormula="", aminoSpecifity="M"
        )
        blocks.modifications["Oxidation"] = mod

    if "Trypsin" not in blocks.enzymes:
        enz = blocks.Enzyme(
            name="Trypsin", expression="[KR][^P]", nTermFormula="H", cTermFormula="OH"
        )
        blocks.enzymes["Trypsin"] = enz

    # Patch sequence to always have 'matches' attribute
    original_init = obj_sequence.Sequence.__init__

    def patched_init(self, *args, **kwargs):
        original_init(self, *args, **kwargs)
        self.matches = []

    mocker.patch.object(obj_sequence.Sequence, "__init__", patched_init)

    parent = MockParent()
    panel = panel_sequence.PanelSequence(parent)
...
def test_init(panel):
    ...
    assert isinstance(panel.currentSequence, obj_sequence.Sequence)
...
def test_setData(panel):
    seq = obj_sequence.Sequence("ACDEF")
...
def test_onSequenceCyclic(panel):
    ...
    assert panel.currentSequence.cyclic

    panel.sequenceCyclic_check.SetValue(False)
    panel.onSequenceCyclic(None)
    assert not panel.currentSequence.cyclic


def test_modifications(panel, mocker):
    seq = obj_sequence.Sequence("MAGA")
    panel.setData(seq)
    ...
    oxidation_mod = blocks.Modification(
        name="Oxidation", gainFormula="O", lossFormula="", aminoSpecifity="M"
    )
    mocker.patch.dict(blocks.modifications, {"Oxidation": oxidation_mod})
...
def test_processing_digestion(panel, mocker):
    seq = obj_sequence.Sequence("MAGAMAGA")
    panel.setData(seq)
    panel.onToolSelected(tool="digest")
    ...
    peptide = obj_sequence.Sequence("MAGA")
    peptide.history = [["digest", 0, 3]]
    peptide.miscleavages = 0
    mocker.patch("mmass.mspy.mod_proteo.digest", return_value=[peptide])

    mocker.patch.object(panel, "getParams", return_value=True)
    mocker.patch("mmass.mspy.mod_proteo.coverage", return_value=50.0)
...
def test_processing_fragmentation(panel, mocker):
    ...
    frag = obj_sequence.Sequence("MA")
    frag.history = [["fragment", 0, 1]]
    frag.fragmentFiltered = False
    mocker.patch("mmass.mspy.Fragment", return_value=[frag])
    mocker.patch("mmass.mspy.mod_proteo.fragmentlosses", return_value=[])
    mocker.patch("mmass.mspy.mod_proteo.fragmentgains", return_value=[])
...
def test_processing_search(panel, mocker):
    ...
    match_pep = obj_sequence.Sequence("MAGA")
    match_pep.history = [["search", 0, 3]]
    mocker.patch.object(obj_sequence.Sequence, "search", return_value=[match_pep])
...
def test_sequenceGrid_logic(panel):
    ...
    seq = obj_sequence.Sequence([], chainType="custom")
    grid.setData(seq)

    grid.items[0].SetValue("M1")

    if "M1" not in blocks.monomers:
        blocks.monomers["M1"] = blocks.Monomer(abbr="M1", name="M1", formula="CH2")
