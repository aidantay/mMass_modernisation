...
def test_labelpoint_empty_signal():
    ...
    signal = np.array([], dtype=np.float64).reshape(0, 2)
    peak = mod_peakpicking.labelpoint(signal, 1000.0)
    assert peak is None
...
def test_labelpoint_output_in_range_property(mz):
...
@given(st.floats(min_value=100.0, max_value=5000.0))
@settings(max_examples=20, deadline=None, suppress_health_check=[HealthCheck.too_slow])
def test_averagine_mass_increases_with_mz_property(mz):
...
@settings(max_examples=20, deadline=None, suppress_health_check=[HealthCheck.too_slow])
def test_deconvolute_singly_charged_property(mz):
...
def test_labelscan_force_quit_in_centroiding():
...
def test_deconvolute_force_quit():
...
def test_labelpeak_centroid_degenerate():
    ...
    signal = np.array(
        [[999.5, 100.0], [1000.0, 1000.0], [1000.5, 100.0]], dtype=np.float64
    )
    peak = mod_peakpicking.labelpeak(signal, mz=1000.0, pickingHeight=0.75)
...
def test_labelscan_no_maxima():
    ...
    signal = np.column_stack(
        [np.linspace(1000.0, 1010.0, 50), np.linspace(100.0, 1000.0, 50)]
    ).astype(np.float64)
...
def test_deconvolute_no_charge_2():
...
def test_labelpoint_with_baseline_array_workaround():
    ...
    # This call demonstrates that baseline handling works correctly.
    result = mod_peakpicking.labelpoint(signal, 1000.0, baseline=baseline)
    assert result is not None
    assert result.mz == 1000.0
    assert result.ai == 1000.0
    assert result.base == 20.0
    assert result.sn == 490.0
...
def test_labelscan_with_baseline_array_workaround():
    ...
    # This call demonstrates that baseline handling works correctly in labelscan.
    result = mod_peakpicking.labelscan(signal, baseline=baseline)
    assert len(result) == 1
    assert result[0].mz == 1000.0
    assert result[0].base == 20.0
...
def test_labelpeak_with_baseline_array_workaround():
    ...
    # This call demonstrates that baseline handling works correctly in labelpeak.
    result = mod_peakpicking.labelpeak(signal, mz=1000.0, baseline=baseline)
    assert result is not None
    assert result.mz == 1000.0
    assert result.base == 20.0
