...
from mmass.mspy import blocks, obj_compound, obj_sequence


def test_init_basic_construction():
    s = obj_sequence.Sequence("ACDEF")
...
def test_init_list():
    s = obj_sequence.Sequence(["A", "C", "D"], chainType="other")
...
def test_init_unknown_monomer():
    with pytest.raises(KeyError):
        obj_sequence.Sequence("AX")


def test_init_cyclic():
    s = obj_sequence.Sequence("ACDEF", cyclic=True)
...
def test_modify():
    ...
    s_cyclic = obj_sequence.Sequence("ACDEF", cyclic=True)
...
def test_setslice():
    s = obj_sequence.Sequence("ACDEF")
    v = obj_sequence.Sequence("AGL", chainType="aminoacids")
...
def test_setslice_errors():
    s = obj_sequence.Sequence("ACDEF")
    with pytest.raises(ValueError, match="Invalid slice!"):
        s[3:1] = obj_sequence.Sequence("A")

    with pytest.raises(TypeError):
        s[1:2] = "ABC"  # Not a sequence object

    with pytest.raises(TypeError):
        s[1:2] = obj_sequence.Sequence("A|G|L", chainType="other")
...
def test_pattern(mocker):
    ...
    real_init = obj_compound.Compound.__init__
    ...
    mocker.patch.object(obj_compound.Compound, "__init__", patched_init)
...
def test_format():
    ...
    # Other chain type
    s_other = obj_sequence.Sequence(["A", "C"], chainType="other")
...
def test_search_advanced():
    s = obj_sequence.Sequence("RKACDEFRK")
    # Trypsin: [KR][^P]
    matches = s.search(
        mass=100.0, charge=1, tolerance=1000.0, enzyme="Trypsin", semiSpecific=True
    )
...
def test_search():
    s = obj_sequence.Sequence("ACDEFGHIKLMN")
    # Search for ACDE
    target = s[0:4]
