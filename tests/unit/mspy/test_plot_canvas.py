...
def canvas_fixture(wx_app, mocker):
    ...
    orig_onSize = plot_canvas.Canvas.onSize
    plot_canvas.Canvas.onSize = mocker.Mock()

    # Instantiate the canvas
    p = plot_canvas.Canvas(parent=parent_frame, size=(800, 600))

    # Restore the original onSize so it can be tested/used later
    plot_canvas.Canvas.onSize = orig_onSize

    # Provide a mock plotBuffer since onSize was skipped during __init__
    p.plotBuffer = mocker.Mock()
...
def setup_canvas_for_tracking(canvas_fixture, mocker):
    ...
    canvas_fixture.pointScale = np.array([1.0, 1.0])
    canvas_fixture.pointShift = np.array([0.0, 0.0])
...
def test_canvas_initialization(wx_app, canvas_fixture):
    ...
    assert isinstance(canvas_fixture, plot_canvas.Canvas)
...
def test_scaleFont(wx_app):
    ...
    base_font = wx.Font(
        10, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, "Arial"
    )

    # Identity Check: scale == 1.0 should return the same object
    scaled_font_1 = plot_canvas._scaleFont(base_font, 1.0)
    assert scaled_font_1 is base_font

    # Magnification Check: scale == 2.0
    scaled_font_2 = plot_canvas._scaleFont(base_font, 2.0)
    ...
    scaled_font_05 = plot_canvas._scaleFont(base_font, 0.5)
    # 10 * 0.5 * 1.3 = 6.5
...
def test_makeAxisTicks(canvas_fixture, lower, upper):
    ...
    for tick in ticks:
        ...
        assert len(tick) == 3
        assert isinstance(tick[0], (float, int, np.float64))
...
def test_coordinate_transformations_roundtrip(canvas_fixture, x, y):
    ...
    canvas_fixture.pointScale = np.array([2.5, -1.5])
    canvas_fixture.pointShift = np.array([10.0, 50.0])
    ...
    np.testing.assert_allclose(roundtrip_pos, [x, y], atol=1e-7)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            (30, 40),
            (20, 60, 40, 60),
        ),  # Top-left to bottom-right (user coords)
        (
            (30, 40),
            (10, 20),
            (20, 60, 40, 60),
        ),  # Bottom-right to top-left (user coords)
        (
            (10, 40),
            (30, 20),
            (20, 60, 40, 60),
        ),  # Bottom-left to top-right (user coords)
    ],
)
def test_pointToClientCoord_positive_dimensions(canvas_fixture, c1, c2, expected):
    ...
    canvas_fixture.pointScale = np.array([2.0, 3.0])
    canvas_fixture.pointShift = np.array([0.0, 0.0])
...
def test_setSize(canvas_fixture):
    ...
    assert np.array_equal(canvas_fixture.plotBoxSize, np.array([800, 600]))
    # x0 = 0.5 * (800 - 800) = 0.0
    # y0 = 600 - 0.5 * (600 - 600) = 600.0
    assert np.array_equal(canvas_fixture.plotBoxOrigin, np.array([0.0, 600.0]))
...
def test_rememberView_defaults(canvas_fixture, mocker):
    ...
    mock_x = mocker.patch.object(
        canvas_fixture, "getCurrentXRange", return_value=(10, 20)
    )
    mock_y = mocker.patch.object(
        canvas_fixture, "getCurrentYRange", return_value=(30, 40)
    )
...
def test_zoom_autoScaleY(canvas_fixture, mocker):
    ...
    mocker.patch.object(canvas_fixture, "rememberView")
    mocker.patch.object(canvas_fixture, "getMaxXRange", return_value=(0, 100))
    mock_get_max_y = mocker.patch.object(
        canvas_fixture, "getMaxYRange", return_value=(10, 200)
    )
...
def test_onLMD_functions(
    canvas_fixture, mocker, mock_event_factory, mouseFnLMB, expected_event, draw_method
):
...
def test_onLMDC_xAxis_autoScaleY_true(canvas_fixture, mocker, mock_event_factory):
    ...
    mocker.patch.object(
        canvas_fixture, "getCursorLocation", side_effect=["xAxis", "blank"]
    )
    mocker.patch.object(canvas_fixture, "getMaxXRange", return_value=(0, 1000))
...
def test_onLMDC_yAxis(canvas_fixture, mocker, mock_event_factory):
    ...
    mocker.patch.object(
        canvas_fixture, "getCursorLocation", side_effect=["yAxis", "blank"]
    )
...
def test_onChar_autoScaleY(canvas_fixture, mocker, mock_event_factory):
    ...
    mocker.patch.object(canvas_fixture, "getMaxXRange", return_value=(0, 1000))
    mock_get_max_y = mocker.patch.object(
        canvas_fixture, "getMaxYRange", return_value=(50, 500)
    )
    mock_draw = mocker.patch.object(canvas_fixture, "draw")
...
def test_drawAxis(patched_canvas, mock_dc):
    ...
    patched_canvas.pointScale = np.array([0.7, -5.0])
    patched_canvas.pointShift = np.array([50.0, 550.0])
...
def test_printout_basics(mocker):
    ...
    po = plot_canvas.Printout(graph=mock_graph, filterSize=1.5, title="Test Print")
...
def test_printout_OnPrintPage_printing(mocker, mock_dc):
    ...
    mock_graph = mocker.Mock()
    po = plot_canvas.Printout(mock_graph, filterSize=2.0)
...
def test_drawIsotopeRuler_detailed(setup_canvas_for_tracking, mocker, platform):
    ...
    mocker.patch.object(
        canvas, "positionUserToScreen", side_effect=lambda pos: (pos[0], pos[1])
    )
    ...
    mocker.patch.object(
        canvas,
        "getPoint",
        side_effect=lambda x, coord: (x, 80.0) if abs(x - 50.0) < 1.0 else (x, 150.0),
    )
...
def test_drawDistanceTracker_detailed(setup_canvas_for_tracking, mocker):
    ...
    assert dc.DrawLine.call_count >= 3
    # xPosDigits=2 -> "20.00"
    canvas.drawInvertedText.assert_called_with(
        mocker.ANY, "20.00", mocker.ANY, mocker.ANY, mocker.ANY
    )
    ...
    # yPosDigits=0 -> "20"
    canvas.drawInvertedText.assert_called_with(
        mocker.ANY, "20", mocker.ANY, mocker.ANY, mocker.ANY
    )
...
def test_mouse_tracking_state_machine(
    setup_canvas_for_tracking,
    mocker,
    mock_event_factory,
    mouse_fn,
    expected_event,
...
def test_onMScroll_early_exit_active_event(
    canvas_fixture, mock_mouse_event_scroll, mocker
):
...
def test_onMScroll_scroll_direction(
...
def test_onMScroll_scale_x_axis_no_cursor(
    setup_scroll, mock_mouse_event_scroll, mocker
):
...
def test_onMScroll_respects_max_zoom(setup_scroll, mock_mouse_event_scroll, mocker):
    ...
    canvas, _ = setup_scroll
    mock_mouse_event_scroll.AltDown.return_value = True
    mock_mouse_event_scroll.GetWheelRotation.return_value = (
        -120
    )  # direction -1 (zoom in)
    mocker.patch.object(canvas, "getCursorPosition", return_value=(150.0, 500.0))
...
def test_onMScroll_respects_x_limits_zoom(
    setup_scroll, mock_mouse_event_scroll, mocker
):
...
def test_onMScroll_respects_x_limits_shift(
    setup_scroll, mock_mouse_event_scroll, mocker
):
...
def test_onMScroll_auto_scale_y(setup_scroll, mock_mouse_event_scroll, mocker):
    ...
    canvas, mock_graphics = setup_scroll
    mock_get_max_y = mocker.patch.object(
        canvas, "getMaxYRange", return_value=(10.0, 500.0)
    )
    mock_draw = mocker.patch.object(canvas, "draw")
    mocker.patch.object(canvas, "rememberView")
...
def test_draw_layout_combinations(
    patched_canvas, mock_dc, mocker, showXPosBar, showYPosBar, showGel, showLegend
):
...
def test_drawIsotopeRuler_with_gel(patched_canvas, mocker, mock_dc):
    ...
    mocker.patch.object(
        patched_canvas, "positionUserToScreen", side_effect=lambda pos: (pos[0], pos[1])
    )
...
def test_drawPointArrow_directions(
    patched_canvas, mocker, mock_dc, direction, expected_poly
):
...
def test_drawAxis_variations(
    patched_canvas, mock_dc, showGrid, showMinorTicks, showZero
):
    ...
    patched_canvas.plotCoords = (50, 50, 750, 550)
    patched_canvas.pointScale = np.array([0.7, -10.0])
    patched_canvas.pointShift = np.array([50.0, 550.0])
