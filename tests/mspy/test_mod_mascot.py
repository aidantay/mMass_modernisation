...
class TestMascotSearch:
    ...
    def test_search_no_regex_match(self, mocker):
        ...
        mock_response = mocker.MagicMock()
        mock_response.status = 200
        mock_response.read.return_value = SAMPLE_SEARCH_RESPONSE_NO_MATCH.encode(
            "utf-8"
        )
...
class TestMascotSave:
    ...
    def test_save_successful_write(self):
        ...
        try:
            ...
            assert (
                "mascot_search_results" in content
                or "mascot_search_results" in content.decode("utf-8", errors="ignore")
            )
        finally:
            if os.path.exists(temp_path):
                os.unlink(temp_path)
...
class TestMascotParameters:
    ...
    def test_parameters_empty_lines_skipped(self, mocker):
        ...
        mock_response = mocker.MagicMock()
        mock_response.status = 200
        mock_response.read.return_value = SAMPLE_PARAMETERS_WITH_EMPTY_LINES.encode(
            "utf-8"
        )
