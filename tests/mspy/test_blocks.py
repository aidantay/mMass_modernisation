...
def test_element_init():
    # Standard case
    el = blocks.element(
        name="Test", symbol="Te", atomicNumber=10, isotopes={1: (10.0, 1.0)}, valence=2
    )
    assert el.name == "Test"
    assert el.symbol == "Te"
    ...
    el2 = blocks.element(
        name="Edge", symbol="Ed", atomicNumber=11, isotopes={0: (12.0, 0.0)}
    )
...
def test_save_load_monomers(tmpdir):
    ...
    blocks.monomers["TEST"] = blocks.monomer(
        abbr="TEST", formula="CH4", name="Methane", category="Test", losses=["H2O"]
    )
    blocks.monomers["INTERNAL"] = blocks.monomer(
        abbr="INTERNAL", formula="H", name="Internal", category="_InternalAA"
    )
