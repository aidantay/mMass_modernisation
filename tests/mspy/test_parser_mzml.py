...
def test_parseScanNumber_valid():
    ...
    assert (
        parser_mzml._parseScanNumber("controllerType=0 controllerNumber=1 scan=456")
        == 456
    )
...
def test_parsePoints_property_based(
    mocker,
    mz_data,
    int_data,
    mz_precision,
...
def test_infoHandler():
    ...
    handler.startElement(
        "cvParam", {"accession": "MS:1000589", "value": "contact@example.com"}
    )
    ...
    handler.startElement(
        "cvParam", {"accession": "MS:1000169", "name": "Mass Spec 2000"}
    )
...
def test_scanlistHandler():
    ...
    handler.startElement(
        "cvParam", {"name": "scan start time", "value": "10.5", "unitName": "minute"}
    )
    ...
    handler.startElement(
        "cvParam", {"name": "scan start time", "value": "630", "unitName": "second"}
    )
    handler.endElement("spectrum")
    ...
    handler.startElement(
        "cvParam", {"name": "scan start time", "value": "5", "unitName": "unknown"}
    )
    handler.endElement("spectrum")
...
def test_runHandler():
    ...
    handler.startElement(
        "cvParam", {"name": "32-bit float"}
    )  # Should be ignored here as not in binaryDataArray
    handler.startElement("binaryDataArray", {})
    handler.startElement("cvParam", {"name": "m/z array"})
    handler.startElement("cvParam", {"name": "32-bit float"})
    handler.startElement("binary", {})
...
def test_parseMZML_integration_tiny():
    ...
    path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "data", "test_tiny.mzML"
    )
...
def test_parseMZML_integration_small():
    ...
    path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "..", "data", "test_small.mzML"
    )
    parser = parser_mzml.parseMZML(path)
...
def test_scanlistHandler_branches():
    ...
    handler.startElement(
        "cvParam", {"name": "scan start time", "value": "1.0", "unitName": "minute"}
    )
    assert handler.data[None]["retentionTime"] == 60.0

    handler.startElement(
        "cvParam", {"name": "scan start time", "value": "120.0", "unitName": "second"}
    )
    assert handler.data[None]["retentionTime"] == 120.0

    handler.startElement(
        "cvParam", {"name": "scan start time", "value": "3.0", "unitName": "unknown"}
    )
...
def test_scanHandler_branches():
    ...
    handler.startElement(
        "cvParam", {"name": "scan start time", "value": "1.0", "unitName": "second"}
    )
    handler.startElement(
        "cvParam", {"name": "scan start time", "value": "1.0", "unitName": "unknown"}
    )
