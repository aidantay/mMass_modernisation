...
from mspy.parser_mzxml import parseMZXML, infoHandler, scanlistHandler, scanHandler, runHandler, stopParsing, _convertRetentionTime
from mspy import obj_scan, obj_peak, obj_peaklist
...
def test_parsePoints_property(parser_instance, points, precision, byte_order, compression, spectrum_type):
...
def test_scanlistHandler_comprehensive():
    ...
    h.startElement('scan', {
        'num': '1', 'msLevel': '2', 'peaksCount': '10', 'polarity': '+',
        'retentionTime': 'PT1S', 'lowMz': '10', 'highMz': '20',
        'basePeakMz': '15', 'basePeakIntensity': '100', 'totIonCurrent': '1000'
    })
...
def test_parseMZXML_load(mocker, tmpdir):
    ...
    mock_handler.data = {1: {
        'points': 'data', 'byteOrder': 'network', 'compression': None, 'precision': 32,
        'title': '', 'scanNumber': 1, 'parentScanNumber': None, 'msLevel': 1,
        'polarity': 1, 'retentionTime': 60.0, 'totIonCurrent': 1000.0,
        'basePeakMZ': 500.0, 'basePeakIntensity': 1000.0, 'precursorMZ': None,
        'precursorIntensity': None, 'precursorCharge': None, 'spectrumType': 'discrete'
    }}
...
def test_parseMZXML_scan(mocker, tmpdir):
    ...
    parser = parseMZXML(path)
    scan_data = {
        'points': None, 'byteOrder': 'network', 'compression': None, 'precision': 32,
        'title': 'Scan 1', 'scanNumber': 1, 'parentScanNumber': None, 'msLevel': 1,
        'polarity': 1, 'retentionTime': 60.0, 'totIonCurrent': 1000.0,
        'basePeakMZ': 500.0, 'basePeakIntensity': 1000.0, 'precursorMZ': None,
        'precursorIntensity': None, 'precursorCharge': None, 'spectrumType': 'discrete'
    }
...
def test_makeScan_direct(parser_instance, mocker):
    scan_data = {
        ...
        'polarity': 1, 'retentionTime': 60.0, 'totIonCurrent': 1000.0,
        'basePeakMZ': 500.0, 'basePeakIntensity': 1000.0, 'precursorMZ': None,
        'precursorIntensity': None, 'precursorCharge': None
    }
