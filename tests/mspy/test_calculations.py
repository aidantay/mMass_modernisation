...
def test_import_calculations(calculations):
    """
    Smoke test to verify that mspy.calculations can be imported.
    This ensures that the compiled C-extension (calculations.so) is present
    and compatible with the current Python environment.
    """
    assert calculations is not None

def test_calculations_module_contents(calculations):
    """
    Verify that the mspy.calculations module has expected attributes.
    This further ensures that the module is correctly loaded.
    """
    # List of functions expected to be exposed by the C-extension
    expected_functions = [
    ...
    for func in expected_functions:
        assert hasattr(calculations, func), "Module mspy.calculations is missing expected function: {}".format(func)


# UNIT & REGRESSION TESTS
# --------------------------------------------------------------------------

# 3.1 signal_interpolate_x
def test_signal_interpolate_x(calculations):
    # Standard case
...
# 3.2 signal_interpolate_y
def test_signal_interpolate_y(calculations):
    # Standard case
...
# 3.3 signal_locate_x
def test_signal_locate_x(calculations):
...
# 3.4 signal_locate_max_y
def test_signal_locate_max_y(calculations):
...
# 3.5 signal_box
def test_signal_box(calculations):
    signal = np.array([[10, 1], [20, 5], [30, 2], [40, 8], [50, 3]], dtype=np.double)
    assert calculations.signal_box(signal) == (10.0, 1.0, 50.0, 8.0)

# 3.6 signal_intensity
def test_signal_intensity(calculations):
...
# 3.10 signal_noise
def test_signal_noise(calculations):
...
# 3.11 signal_local_maxima
def test_signal_local_maxima(calculations):
...
# 3.12 signal_crop
def test_signal_crop(calculations):
...
# 3.13 signal_offset
def test_signal_offset(calculations):
...
# 3.14 signal_multiply
def test_signal_multiply(calculations):
    signal = np.array([[10, 1], [20, 2]], dtype=np.double)
...
# 3.15 signal_normalize
def test_signal_normalize(calculations):
...
# 3.16 signal_smooth_ma
def test_signal_smooth_ma(calculations):
...
# 3.17 signal_smooth_ga
def test_signal_smooth_ga(calculations):
    signal = np.array([[0, 0], [1, 10], [2, 0], [3, 10], [4, 0]], dtype=np.double)
...
# 3.18 signal_combine
def test_signal_combine(calculations):
...
# 3.19 signal_overlay
def test_signal_overlay(calculations):
    signalA = np.array([[10, 10], [20, 20]], dtype=np.double)
...
# 3.20 signal_subtract
def test_signal_subtract(calculations):
    signalA = np.array([[10, 10], [20, 20]], dtype=np.double)
    signalB = np.array([[15, 5], [25, 5]], dtype=np.double)
...
# 3.21 signal_subbase
def test_signal_subbase(calculations):
...
# 3.22 signal_rescale
def test_signal_rescale(calculations):
    signal = np.array([[10, 1], [20, 2]], dtype=np.double)
...
# 3.24 signal_gaussian
def test_signal_gaussian(calculations):
...
# 3.25 signal_lorentzian
def test_signal_lorentzian(calculations):
...
# 3.26 signal_gausslorentzian
def test_signal_gausslorentzian(calculations):
...
# 3.28 signal_profile_to_raster
def test_signal_profile_to_raster(calculations):
    peaks = np.array([[100, 100, 1], [200, 50, 1]], dtype=np.double)
    raster = np.linspace(50, 250, 1000, dtype=np.double)
    
    # Shape 0: Gaussian
    profile0 = calculations.signal_profile_to_raster(peaks, raster, 0.0, 0)
    assert len(profile0) == len(raster)
    idx1 = np.argmin(np.abs(profile0[:,0] - 100))
    assert_allclose(profile0[idx1, 1], 100.0, rtol=1e-2)
    
    # Shape 1: Lorentzian
    profile1 = calculations.signal_profile_to_raster(peaks, raster, 0.0, 1)
    assert len(profile1) == len(raster)
    idx1 = np.argmin(np.abs(profile1[:,0] - 100))
    assert_allclose(profile1[idx1, 1], 100.0, rtol=1e-2)
    
    # Shape 2: Half-Gaussian half-Lorentzian
    profile2 = calculations.signal_profile_to_raster(peaks, raster, 0.0, 2)
    assert len(profile2) == len(raster)
    idx1 = np.argmin(np.abs(profile2[:,0] - 100))
    assert_allclose(profile2[idx1, 1], 100.0, rtol=1e-2)

# 3.29 signal_profile
def test_signal_profile(calculations):
    peaks = np.array([[100, 100, 1], [200, 50, 1]], dtype=np.double)
    # Test all shapes
    for shape in [0, 1, 2]:
...
# 3.30 formula_composition
def test_formula_composition(calculations):
    min_comp = (0, 0)
    max_comp = (2, 5)
...
def test_signal_interpolate_x_property(calculations, x1, y1, x2, y2, v):
...
def test_signal_interpolate_y_property(calculations, x1, y1, x2, y2, v):
...
def test_signal_locate_x_property(calculations, signal, x):
    # Ensure signal is contiguous and double
...
def test_signal_locate_max_y_property(calculations, signal):
    signal = np.ascontiguousarray(signal, dtype=np.double)
    if len(signal) == 0:
        # Known issue: empty array might be risky, but let's see if it crashes.
        # Based on previous trial it returns 0.
...
def test_signal_box_property(calculations, signal):
    signal = np.ascontiguousarray(signal, dtype=np.double)
    if len(signal) == 0:
        # If it crashes, it will be caught.
        try:
...
def test_signal_intensity_property(calculations, signal, x):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_centroid_property(calculations, signal, x, h):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_width_property(calculations, signal, x, h):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_area_property(calculations, signal):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
# 3.10 signal_noise
@given(
    signal=signals(min_size=1) # signal_noise needs at least 1 element for malloc
)
def test_signal_noise_property(calculations, signal):
    signal = np.ascontiguousarray(signal, dtype=np.double)
    level, width = calculations.signal_noise(signal)
...
def test_signal_local_maxima_property(calculations, signal):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_crop_property(calculations, signal, mX, MX):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_offset_property(calculations, signal, v1, v2):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_multiply_property(calculations, signal, v1, v2):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_normalize_property(calculations, signal):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_smooth_ma_property(calculations, signal, window, cycles):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_smooth_ga_property(calculations, signal, window, cycles):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_combine_property(calculations, signalA, signalB):
...
def test_signal_overlay_property(calculations, signalA, signalB):
    signalA = np.ascontiguousarray(signalA, dtype=np.double)
    signalB = np.ascontiguousarray(signalB, dtype=np.double)
...
def test_signal_subtract_property(calculations, signalA, signalB):
    signalA = np.ascontiguousarray(signalA, dtype=np.double)
    signalB = np.ascontiguousarray(signalB, dtype=np.double)
...
def test_signal_subbase_property(calculations, signalA, signalB):
    signalA = np.ascontiguousarray(signalA, dtype=np.double)
    signalB = np.ascontiguousarray(signalB, dtype=np.double)
...
def test_signal_rescale_property(calculations, signal, scaleX, scaleY, shiftX, shiftY):
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_filter_property(calculations, signal, resol):
    assume(resol > 0)
    signal = np.ascontiguousarray(signal, dtype=np.double)
...
def test_signal_gaussian_property(calculations, x, minY, maxY, fwhm, points):
...
def test_signal_lorentzian_property(calculations, x, minY, maxY, fwhm, points):
...
def test_signal_gausslorentzian_property(calculations, x, minY, maxY, fwhm, points):
...
def test_signal_profile_to_raster_property(calculations, peaks, raster, noise, shape):
...
def test_signal_profile_property(calculations, peaks, points, noise, shape):
    peaks = np.ascontiguousarray(peaks, dtype=np.double)
...
def test_formula_composition_property(calculations, min_comp, max_comp, masses, loMass, hiMass, limit):
...
# 3.31 Miscellaneous edge cases
@pytest.mark.skip(reason="Segfaults due to lack of type checking in C extension")
def test_invalid_inputs(calculations):
    # None input should be handled (it currently segfaults, so this test will fail if not skipped or fixed)
    # We are told to assert it handles it gracefully, but we know it doesn't.
    # The contingency plan says to skip if it segfaults.
