...
def test_md_types():
    ...
    # standard (floor)
    assert mspy.mod_basics.md(
        mass, mdType="standard", rounding="floor"
    ) == pytest.approx(0.12345)

    # standard (ceil)
    assert mspy.mod_basics.md(
        mass, mdType="standard", rounding="ceil"
    ) == pytest.approx(1000.12345 - 1001.0)

    # relative
    expected_rel = 1e6 * (1000.12345 - 1000.0) / 1000.12345
    assert mspy.mod_basics.md(
        mass, mdType="relative", rounding="floor"
    ) == pytest.approx(expected_rel)
...
def test_md_kendrick():
    ...
    assert mspy.mod_basics.md(
        mass, mdType="kendrick", kendrickFormula="CH2"
    ) == pytest.approx(expected_km)
...
def test_mz_current_charge():
    ...
    h_mass = h.mass()
    agent_mass = (
        h_mass[0] - mspy.mod_basics.ELECTRON_MASS,
        h_mass[1] - mspy.mod_basics.ELECTRON_MASS,
    )
...
def test_mz_electron():
    ...
    assert mspy.mod_basics.mz(
        1000.0, -1, agentFormula="e", agentCharge=-1
...
def test_frules_carbonless():
    ...
    # NOPS rules do not.
    assert (
        mspy.mod_basics.frules("H2O", rules=["HC", "NOPSC", "NOPS", "RDBE", "RDBEInt"])
        is True
    )
