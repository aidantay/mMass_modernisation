...
@settings(max_examples=20, deadline=1000) # Limit examples and increase deadline for stability in legacy CI
@given(st.lists(st.tuples(st.floats(min_value=-1e10, max_value=1e10, allow_infinity=False, allow_nan=False), 
    ...
    try:
        ...
        for endian, precision in configs:
            ...
            for i in range(len(points)):
                ...
                assert parsed[i][0] == pytest.approx(points[i][0], rel=1e-5 if precision == 32 else 1e-12)
                assert parsed[i][1] == pytest.approx(points[i][1], rel=1e-5 if precision == 32 else 1e-12)
            ...
            for i in range(len(points)):
                assert parsed_cont[i, 0] == pytest.approx(points[i][0], rel=1e-5 if precision == 32 else 1e-12)
                assert parsed_cont[i, 1] == pytest.approx(points[i][1], rel=1e-5 if precision == 32 else 1e-12)
...
def test_parseMZDATA_load(mocker):
    ...
    mock_handler.data = {
        1: {'mzData': 'data1', 'mzEndian': 'little', 'mzPrecision': 32, 'intData': 'data2', 'intEndian': 'little', 'intPrecision': 32, 'other': 'value'},
        2: {'mzData': 'data3', 'mzEndian': 'little', 'mzPrecision': 64, 'intData': 'data4', 'intEndian': 'little', 'intPrecision': 64, 'other': 'value2'}
    }
