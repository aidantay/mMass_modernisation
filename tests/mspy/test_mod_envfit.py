...
def test_setup_verification(
    envfit_obj, mock_check_force_quit, dummy_signal, dummy_baseline, dummy_points
):
...
def test_topoints_autoAlign_false(envfit_obj, mocker):
    ...
    mocker.patch.object(
        envfit_obj, "_makeModels", return_value=(numpy.array([[1.0]]), [0])
    )
...
def test_topoints_leastSquare_zero(envfit_obj, mocker):
    ...
    mocker.patch.object(
        envfit_obj, "_makeModels", return_value=(numpy.array([[1.0], [1.0]]), [0, 1])
    )
...
def test_topoints_success(envfit_obj, mocker):
    ...
    exchanged = [0, 1]
    mocker.patch.object(
        envfit_obj, "_makeModels", return_value=(models_matrix, exchanged)
    )

    # _leastSquare returns abundances
    mocker.patch.object(
        envfit_obj, "_leastSquare", return_value=numpy.array([100.0, 50.0])
    )
...
def test_alignData_multiple_points_per_peak(envfit_obj, mocker):
    ...
    envfit_obj.data = numpy.array([[19.0, 100.0], [19.0, 200.0], [20.0, 50.0]])
