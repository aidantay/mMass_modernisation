...
def test_onClose_safe(mocker, mascot_panel):
    ...
    mock_destroy = mocker.patch.object(mascot_panel, 'Destroy', wraps=mascot_panel.Destroy)
...
def test_onModificationSelected(mascot_panel):
    ...
    assert "Variable modifications: (2)" in mascot_panel.paramPMFVariableMods_label.GetLabel()
...
def test_onHiddenModifications(mocker, mascot_panel):
    ...
    mascot_panel.currentParams = {'MODS': [], 'HIDDEN_MODS': []} # Required for updateModification
...
def test_updateServerParams_success(mocker, mascot_panel):
    ...
    mascot_panel.currentParams = {
        'DB': ['DB1'], 'TAXONOMY': ['TAX1'], 'CLE': ['CLE1'],
        'MODS': ['MOD1'], 'HIDDEN_MODS': ['HMOD1'],
        'INSTRUMENT': ['INST1'], 'QUANTITATION': ['QUANT1']
    }
...
def test_updateModification(mascot_panel):
    ...
    selected_strings = [mascot_panel.paramPMFFixedMods_listbox.GetString(s) for s in selections]
