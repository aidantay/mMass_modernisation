...
def test_makeGUI(calculator_panel):
    ...
    assert calculator_panel.mainSizer.GetItem(0).GetSizer() is None # It's a panel containing a sizer
    ...
    assert calculator_panel.mainSizer.GetItem(3).GetSizer() == calculator_panel.patternSizer
...
def test_onToolSelected(calculator_panel):
    ...
    # Check button icon
    assert calculator_panel.summary_butt.GetBitmapLabel().IsSameAs(images.lib['massCalculatorSummaryOn'])
    ...
    assert calculator_panel.mainSizer.IsShown(3) is False
    # Check button icon
    assert calculator_panel.ionseries_butt.GetBitmapLabel().IsSameAs(images.lib['massCalculatorIonSeriesOn'])
    ...
    # Check button icon
    assert calculator_panel.pattern_butt.GetBitmapLabel().IsSameAs(images.lib['massCalculatorPatternOn'])
...
def test_onCollapse(calculator_panel):
    ...
    assert calculator_panel.patternCollapse_butt.GetBitmapLabel().IsSameAs(images.lib['arrowsDown'])

    # Collapse
    calculator_panel.onCollapse(None)
    assert calculator_panel.patternSizer.IsShown(1) is False
    assert calculator_panel.patternCollapse_butt.GetBitmapLabel().IsSameAs(images.lib['arrowsRight'])
...
def test_setData(calculator_panel, patched_config, mocker):
    ...
    mock_run_ions = mocker.patch.object(calculator_panel, 'runIonSeries', wraps=calculator_panel.runIonSeries)
    mock_run_pattern = mocker.patch.object(calculator_panel, 'runPattern', wraps=calculator_panel.runPattern)
    
    calculator_panel.setData(formula="C6H12O6", charge=1, fwhm=0.5, intensity=5000, baseline=10)
...
def test_onToolSelected_ids(calculator_panel, mocker):
    ...
    from gui.ids import ID_massCalculatorSummary, ID_massCalculatorIonSeries, ID_massCalculatorPattern
    
    for tool_id, tool_name in [(ID_massCalculatorSummary, 'summary'),
