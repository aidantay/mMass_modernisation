...
from gui import mwx
from gui.panel_sequence import panelSequence, sequenceCanvas, sequenceGrid
from gui.ids import *
...
def panel(wx_app, mocker):
    ...
    if 'A' not in mspy.monomers:
        mspy.monomers['A'] = mspy.blocks.monomer(abbr='A', name='Alanine', formula='C3H5NO', category='_InternalAA')
    if 'M' not in mspy.monomers:
        mspy.monomers['M'] = mspy.blocks.monomer(abbr='M', name='Methionine', formula='C5H9NOS', category='_InternalAA')
    
    if 'Oxidation' not in mspy.modifications:
        mod = mspy.blocks.modification(name='Oxidation', gainFormula='O', lossFormula='', aminoSpecifity='M')
        mspy.modifications['Oxidation'] = mod
    
    if 'Trypsin' not in mspy.enzymes:
        enz = mspy.blocks.enzyme(name='Trypsin', expression='[KR][^P]', nTermFormula='H', cTermFormula='OH')
...
def test_modifications(panel, mocker):
    ...
    oxidation_mod = mspy.blocks.modification(name='Oxidation', gainFormula='O', lossFormula='', aminoSpecifity='M')
