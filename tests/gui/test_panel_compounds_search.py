...
def test_onItemSendToMassCalculator(panel, mocker):
    ...
    panel.onItemSendToMassCalculator(None)
    panel.parent.onToolsMassCalculator.assert_called_with(formula="H2O", charge=1, agentFormula='H', agentCharge=1)
    
    # Case 3: Radical selection
    panel.compoundsList.GetItemData.return_value = 1
    panel.onItemSendToMassCalculator(None)
    panel.parent.onToolsMassCalculator.assert_called_with(formula="H2O", charge=1, agentFormula='e', agentCharge=-1)
