...
from gui.panel_processing import panelProcessing, dlgPresetsName
from gui.ids import *
...
def test_onPresetsSelected(panel, mocker):
    ...
    presets_data = {
        ...
        'peakpicking': {
            'snThreshold': 5, 'absIntThreshold': 10, 'relIntThreshold': 0.05,
            'pickingHeight': 0.5, 'baseline': False, 'smoothing': False,
            'deisotoping': False, 'removeShoulders': True
        },
        'deisotoping': {
            'maxCharge': 5, 'massTolerance': 0.05, 'intTolerance': 0.1,
            'isotopeShift': 0.01, 'removeIsotopes': True, 'removeUnknown': True,
            'setAsMonoisotopic': False, 'labelEnvelope': 'centroid', 'envelopeIntensity': 'sum'
        },
        'deconvolution': {
            'massType': 1, 'groupWindow': 0.1, 'groupPeaks': False, 'forceGroupWindow': True
        },
        'batch': {
            'swap': True, 'math': True, 'crop': True, 'baseline': True,
            'smoothing': True, 'peakpicking': True, 'deisotoping': True, 'deconvolution': True
        }
    }
    ...
    assert panel.deisotopingLabelEnvelopeTool_choice.GetSelection() == 2 # Envelope Centroid
    assert panel.deisotopingEnvelopeIntensity_choice.GetSelection() == 1 # Summed Isotopes
...
def sync_thread(mocker):
    ...
    def mock_init(self, group=None, target=None, name=None, args=(), kwargs=None, verbose=None):
        self._target = target
        self._args = args
...
def test_runApplyBatch(panel, mocker, sync_thread):
    ...
    # Select the item
    panel.batchDocumentsList.SetItemState(0, wx.LIST_STATE_SELECTED, wx.LIST_STATE_SELECTED)
