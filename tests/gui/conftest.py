...
def wx_app():
    ...
    try:
        ...
        if not hasattr(wx.Image, "SetOptionInt"):
            wx.Image.SetOptionInt = lambda self, name, value: self.SetOption(name, str(value))
        images.loadImages()
    except Exception:
        # Fallback: populate images.lib with dummy bitmaps so tests don't fail on KeyError
        dummy_bitmap = wx.Bitmap(16, 16)
        keys = [
            'bulletsDocument', 'bulletsAnnotationsOn', 'bulletsAnnotationsOff',
            'bulletsSequenceOn', 'bulletsSequenceOff', 'bulletsNotationOn',
            'bulletsNotationOff', 'documentsAdd', 'documentsDelete', 'bgrBottombar'
        ]
        for key in keys:
