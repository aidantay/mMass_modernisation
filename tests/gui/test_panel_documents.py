import pytest
import wx
from gui.panel_documents import panelDocuments, documentsTree, fileDropTarget
from gui.ids import *
...
class DummyDocument(object):
    ...
    def __init__(self, title='', visible=True, colour=(0, 0, 255), dirty=False):
        ...
        self.offset = [0, 0]
        self.style = wx.SOLID
        self.spectrum = type('DummySpectrum', (object,), {'hasprofile': lambda self: True})()
        self.annotations = []
        self.sequences = []
        self.bulletIndex = -1
...
class DummyAnnotation(object):
    ...
    def __init__(self, label='', mz=0.0, formula=None, charge=1, radical=False, theoretical=None):
        self.label = label
        self.mz = mz
...
class DummyMatch(object):
    """Dummy match class."""
    def __init__(self, label='', mz=0.0, theoretical=None, formula=None, charge=1, radical=False, sequenceRange=None):
        self.label = label
        self.mz = mz
        self.theoretical = theoretical
        self.formula = formula
...
def test_documentsTree_visual_updates(wx_app, mocker):
    ...
    tree.updateDocumentColour(seq_item) # pass child item to check getParentItem(item, 1)
...
def test_panelDocuments_initialization(wx_app, mocker):
    ...
    mocker.patch('gui.panel_documents.config.main', {'mzDigits': 4, 'ppmDigits': 2, 'errorUnits': 'ppm'})
    ...
    sizer_items = [item.GetWindow() for item in panel.GetSizer().GetChildren() if item.IsWindow()]
    ...
    toolbar_items = [item.GetWindow() for item in inner_sizer.GetChildren() if item.IsWindow()]
...
def test_panelDocuments_onLMD(wx_app, mocker):
    ...
    mocker.patch.object(tree, 'HitTest', return_value=(doc_item, wx.TREE_HITTEST_ONITEMICON))
...
def test_panelDocuments_onRMU(wx_app, mocker):
    ...
    appended_ids = [call[0][0] for call in menu.Append.call_args_list if len(call[0]) > 0]
...
def test_panelDocuments_SendToHandlers(wx_app, mocker):
    ...
    doc_data = DummyDocument(title='Doc 1')
    seq_data = DummySequence(title='Seq 1')
    annot_data = DummyAnnotation(label='Annot 1', mz=100.0, formula="C1H1", charge=1, radical=False)
    match_data = DummyMatch(label='Match 1', mz=200.0, formula="C2H2", charge=2, radical=True, sequenceRange=[10, 20])
    doc_data.sequences = [seq_data]
    doc_data.annotations = [annot_data]
    seq_data.matches = [match_data]
    documents = [doc_data]
    ...
    mocker.patch('gui.panel_documents.config.main', {'mzDigits': 4, 'ppmDigits': 2, 'errorUnits': 'ppm'})
    mocker.patch('gui.panel_documents.config.envelopeFit', {'loss': 'H', 'gain': 'H{2}'})
    panel = panelDocuments(parent, documents)
    tree = panel.documentTree
    ...
    tree.SelectItem(annot_item)
    panel.onSendToMassCalculator()
    parent.onToolsMassCalculator.assert_called_with(formula="C1H1", charge=1, agentFormula='H', agentCharge=1)
    
    # Match (radical)
    tree.SelectItem(match_item)
    panel.onSendToMassCalculator()
    parent.onToolsMassCalculator.assert_called_with(formula="C2H2", charge=2, agentFormula='e', agentCharge=-1)
    
    # 2. onSendToMassToFormula
    # Annotation
    tree.SelectItem(annot_item)
    panel.onSendToMassToFormula()
    parent.onToolsMassToFormula.assert_called_with(mass=100.0, charge=1, agentFormula='H')
    
    # Match (radical)
    tree.SelectItem(match_item)
    panel.onSendToMassToFormula()
    parent.onToolsMassToFormula.assert_called_with(mass=200.0, charge=2, agentFormula='e')
    ...
    tree.SelectItem(match_item)
    panel.onSendToEnvelopeFit()
    parent.onToolsEnvelopeFit.assert_called_with(formula="C2H2", charge=2, scale=[0, 10])
