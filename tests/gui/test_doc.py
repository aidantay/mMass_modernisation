...
def mock_config(mocker):
    ...
    mocker.patch.dict('gui.config.main', {
        'mzDigits': 4,
        'intDigits': 0,
        'ppmDigits': 1,
        'dataPrecision': 32
...
def test_annotation_delta():
    ...
    annot = gui.doc.annotation(label="Test", mz=100.1234, ai=1000.0, theoretical=100.1200)
...
def test_document_backup_restore(wx_app, mock_config, sample_spectrum):
    ...
    assert doc.sequences[0].title == "ShouldNotBeRestored" # Only matches are restored for sequences
...
def test_document_msd(mock_config, sample_spectrum):
    ...
    doc.annotations = [
        gui.doc.annotation(label="Annot1", mz=150.0, ai=50.0, formula="H2O", charge=1, radical=True, theoretical=150.005)
    ]
    
    # Add a peak with optional attributes
    p_opt = mspy.peak(mz=300.1, ai=500.0, base=10.0, sn=15.5, charge=2, isotope=0, fwhm=0.01, group="Group1")
    ...
    m_opt = gui.doc.match(label="y1", mz=147.1, ai=200.0, base=5.0, charge=1, radical=True, theoretical=147.11, formula="C6H11NO3")
...
def test_document_report(mock_config, sample_spectrum, mocker):
    ...
    doc.annotations = [
        gui.doc.annotation(label="Annot1", mz=150.0, ai=500.0, theoretical=150.005, formula="H2O", charge=1, radical=True)
    ]
    ...
    seq.matches = [
        gui.doc.match(label="Match1", mz=200.0, ai=1000.0, theoretical=200.001, charge=2, radical=False, formula="C10H20")
    ]
...
def test_parseMSD_integration(tmp_path, mock_config, sample_spectrum, mocker):
    ...
    # Add match
    m = gui.doc.match(label="y1", mz=147.1, ai=200.0, theoretical=147.11, formula="C6H11NO3")
    m.sequenceRange = [7, 7]
    seq.matches = [m]
