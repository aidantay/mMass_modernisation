...
def test_set_data(panel, mocker):
    ...
    assert panel.ionization_choice.GetStringSelection() == 'Na+' # In the code choices = ['M', 'M*', 'H+', 'Na+', 'K+', 'Li+', 'NH4+']
...
def test_get_params(panel, mocker):
    ...
    assert config.massToFormula['ionization'] == 'H' # choices = ['', 'e', 'H', 'Na', 'K', 'Li', 'NH4']
...
@pytest.mark.parametrize("server_id, server_name", [
