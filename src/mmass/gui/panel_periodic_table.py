...
class panelPeriodicTable(wx.Frame):
    ...
    def onElementSelected(self, evt=None):
        ...
        # get current element
            ...
            # show element info
            self.elementName.SetLabel(
                f"{mspy.elements[element].name} {element} ({mspy.elements[element].atomicNumber})"
            )
            self.elementMass.SetLabel(
                "Mo. mass: {:.6f} Da\nAv. mass: {:.6f} Da".format(
                    *mspy.elements[element].mass
                )
            )
    ...
    def onWiki(self, evt):
        ...
        # show selected group
            ...
            link = f"http://en.wikipedia.org/wiki/{groups[self.currentGroup]}"
