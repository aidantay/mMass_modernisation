...
class panelDocuments(wx.Panel):
    ...
    def onRMU(self, evt):
        ...
        # get object index
        self._getDocumentIndex(item)
        self._getSequenceIndex(item)
...
class documentsTree(wx.TreeCtrl):
    ...
    def _makeColourBullet(self, colour, filled=True):
        ...
        # clear background
        if wx.Platform != "__WXMAC__":
            dc.SetBackground(wx.Brush(mwx.DOCTREE_COLOUR, wx.BRUSHSTYLE_SOLID))
            dc.Clear()

        # set pen and brush
        if filled:
            pencolour = [max(x - 70, 0) for x in colour]
            dc.SetPen(wx.Pen(pencolour, 1, wx.PENSTYLE_SOLID))
            dc.SetBrush(wx.Brush(colour, wx.BRUSHSTYLE_SOLID))
        else:
