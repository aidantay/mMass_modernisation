...
class panelProcessing(wx.Frame):
    ...
    def onMathChanged(self, evt=None):
        ...
        if (
            ...
            or self.mathOperationNorm_radio.GetValue()
        ):
            self.mathSpectrumB_choice.Disable()
            self.mathMultiply_value.Disable()
        elif (
            self.mathOperationCombine_radio.GetValue()
            or self.mathOperationOverlay_radio.GetValue()
            or self.mathOperationSubtract_radio.GetValue()
        ):
    ...
    def onApply(self, evt=None):
        ...
        # check document
        if self.currentDocument is None and self.currentTool not in ("math", "batch"):
            wx.Bell()
            return
