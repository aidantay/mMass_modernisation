...
class PanelMatch(wx.Frame):
    ...
    def makeErrorCanvas(self) -> None:
        ...
        self.errorCanvas.setProperties(
            yLabel="error ({})".format(config.match["units"])
        )
    ...
    def makeMatchSummary(self) -> None:
        ...
        for item in self.currentData:
            if item[-1]:
                sumMatched += 1
        label = f"Number of {itemName} matched"
        ...
        for item in self.currentData:
            for n in item[-1]:
                ...
                if mz in buff:
                    ...
                    del buff[mz]
        value = (
            "{:0.0f} {}".format(100 * matchedInt / totalInt, "%") if totalInt else "0 %"
        )
        ...
        # get ion series
            ...
            for item in self.currentData:
                ...
                if frag.fragmentSerie not in (
                    ...
                    "n-ladder",
                    "c-ladder",
                ) or "break" in [x[0] for x in frag.history]:
                    continue
            ...
            for serie in sorted(series.keys()):
                ...
                label = f'Ion serie matches for "{serie}"'
                self.currentSummary.append((label, value))
