...
class PanelSequence(wx.Frame):
    ...
    def makeFragmentPanel(self):
        ...
        self.fragmentGainCO_check.SetToolTip("Allowed for 'b' and 'c' ions only.")
        ...
        self.fragmentScrambling_check.SetToolTip("Allow non-direct sequencing.")
    ...
    def onAddModification(self, evt) -> None:
        ...
        # set residual modification
            ...
            pos = position.split(" ")
            amino = str(pos[1]) if pos[0] == "All" else int(pos[1]) - 1
    ...
    def updateSequenceInfo(self) -> None:
        ...
        if len(self.currentSequence) and self.currentSequence.chainType == "aminoacids":
            ...
            if selection[0] == selection[1]:
                ...
                label += f"     Cursor: {selection[0] + 1}-{min(selection[1], len(self.currentSequence))}"
    ...
    def updateAvailableResidues(self) -> None:
        ...
        for monomer in self.currentSequence:
            name = f"{mspy.monomers[monomer].name} ({monomer})"
    ...
    def updateDigestList(self) -> None:
        ...
        for index, item in enumerate(self.currentDigest):
            # filter data
            if (self._digestFilter == 1 and item[5] is None) or (
                self._digestFilter == -1 and item[5] is not None
            ):
                continue
    ...
    def updateFragmentsList(self) -> None:
        ...
        for index, item in enumerate(self.currentFragments):
            # filter data
            if (self._fragmentsFilter == 1 and item[5] is None) or (
                self._fragmentsFilter == -1 and item[5] is not None
            ):
                continue
    ...
    def updateCoverage(self) -> None:
        ...
        # set new label
        label = f"Coverage: {matchedCoverage:0.0f}/{totalCoverage:0.0f}"
        label += " %"
...
class SequenceCanvas(wx.TextCtrl):
    ...
    def _updateCanvas(self) -> None:
        ...
        for pos in modifications:
            x, _y = self._positionSequenceToEditor([pos, pos])
