...
def appInit():
    ...
    # set WIN
    elif wx.Platform == "__WXMSW__" or wx.Platform == "__WXGTK__":
...
class sortListCtrl(wx.ListCtrl):
    ...
    def _onColClick(self, evt):
        ...
        direction = -1 * self._currentDirection if oldCol == newCol else LISTCTRL_SORT
    ...
    def _sortItems(self, item1, item2):
        ...
        if comp == 0 and self._secondarySortColumn is not None:
            a = item1[self._secondarySortColumn]
            b = item2[self._secondarySortColumn]
            comp = (a > b) - (a < b)
...
class validator(wx.Validator):
    ...
    def OnChar(self, evt):
        ...
        if (
            ...
            or (key == 99 and evt.CmdDown())
            or (key == 118 and evt.CmdDown())
        ):
            evt.Skip()
            return
        ...
        # int only
        if (
            (self.flag == "int" and chr(key) in "-0123456789eE")
            or (self.flag == "intPos" and chr(key) in "0123456789eE")
            or (self.flag == "float" and (chr(key) in "-0123456789.eE"))
            or (self.flag == "floatPos" and (chr(key) in "0123456789.eE"))
        ):
            evt.Skip()
            return
