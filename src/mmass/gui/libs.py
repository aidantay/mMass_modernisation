...
# ENSURE DEFAULT LIBS ARE AVAILABLE
# ---------------------------------
for item in (
    "monomers.xml",
    "modifications.xml",
...
def savePresets(path: Path | str = config.confdir / "presets.xml") -> bool:
    ...
    for name in sorted(presets["operator"].keys()):
        ...
        buff += f'      <param name="operator" value="{_escape(item["operator"])}" type="str" />\n'
        buff += f'      <param name="contact" value="{_escape(item["contact"])}" type="str" />\n'
        buff += f'      <param name="institution" value="{_escape(item["institution"])}" type="str" />\n'
        buff += f'      <param name="instrument" value="{_escape(item["instrument"])}" type="str" />\n'
        buff += "    </presets>\n\n"
    buff += "  </operator>\n\n"
    ...
    for name in sorted(presets["processing"].keys()):
        ...
        buff += (
            '        <param name="isotopeShift" value="{:f}" type="float" />\n'.format(
        ...
        buff += (
            '        <param name="envelopeIntensity" value="{}" type="str" />\n'.format(
    ...
    for name in sorted(presets["modifications"].keys()):
        ...
        for mod in presets["modifications"][name]:
            buff += f'      <modification name="{mod[0]}" position="{mod[1]}" type="{mod[2]}" />\n'
        buff += "    </presets>\n\n"
    buff += "  </modifications>\n\n"
...
def saveReferences(path: Path | str = config.confdir / "references.xml") -> bool:
    ...
    for group in sorted(references.keys()):
        buff += f'  <group name="{_escape(group)}">\n'
        for ref in references[group]:
            buff += f'    <reference name="{_escape(ref[0])}" mass="{ref[1]:f}" />\n'
        buff += "  </group>\n\n"
...
def saveCompounds(path: Path | str = config.confdir / "compounds.xml") -> bool:
    ...
    for group in sorted(compounds.keys()):
        ...
        for name, compound in sorted(compounds[group].items()):
            buff += f'    <compound name="{_escape(name)}" formula="{compound.expression}">{_escape(compound.description)}</compound>\n'
        buff += "  </group>\n\n"
...
def saveMascot(path: Path | str = config.confdir / "mascot.xml") -> bool:
    ...
    for name in sorted(mascot.keys()):
        buff += f'   <server name="{_escape(name)}">\n'
        buff += f'     <param name="protocol" value="{_escape(mascot[name]["protocol"])}" type="str" />\n'
        buff += f'     <param name="host" value="{_escape(mascot[name]["host"])}" type="str" />\n'
        buff += f'     <param name="path" value="{_escape(mascot[name]["path"])}" type="str" />\n'
        buff += f'     <param name="search" value="{_escape(mascot[name]["search"])}" type="str" />\n'
        buff += f'     <param name="results" value="{_escape(mascot[name]["results"])}" type="str" />\n'
        buff += f'     <param name="export" value="{_escape(mascot[name]["export"])}" type="str" />\n'
        buff += f'     <param name="params" value="{_escape(mascot[name]["params"])}" type="str" />\n'
        buff += "   </server>\n\n"
