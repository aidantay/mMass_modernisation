...
class panelMassCalculator(wx.Frame):
    ...
    def onSave(self, evt):
        ...
        document.notes += "FWHM: {}\n\n".format(config.massCalculator["patternFwhm"])
        document.notes += f"Compound: {self.currentCompound.formula()}\n"
        document.notes += f"Monoisotopic mass: {mass[0]}\n"
        document.notes += f"Average mass: {mass[1]}\n\n"
        document.notes += f"Ion: {ion}\n"
        document.notes += f"Monoisotopic m/z: {mz[0]}\n"
        document.notes += f"Average m/z: {mz[1]}\n"
