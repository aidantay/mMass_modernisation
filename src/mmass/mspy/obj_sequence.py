...
class Sequence:
    ...
    def _get_slice(self, start, stop):
        ...
        for mod in parent.modifications:
            if (
                (mod[1] == "nTerm" and start == 0)
                or (mod[1] == "cTerm" and (stop == -1 or stop == len(parent)))
    ...
    def _set_slice(self, start, stop, value) -> None:
        ...
        if value.modifications or value.labels:
            raise NotImplementedError(
                "Sequence _set_slice doesn't support modifications and labels."
            )
    ...
    def format(self, template="S [m]"):
        ...
        if "h" in template:
            for item in self.history[1:]:
                if item[0] == "slice":
                    keys["h"] += f"[{item[1] + 1}-{item[2]}]"
                elif item[0] == "break":
                    keys["h"] += f"[{item[1] + 1}|{item[2] + 1}]"
