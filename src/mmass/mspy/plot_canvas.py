...
class canvas(wx.Window):
    ...
    def onPaint(self, evt):
        ...
        wx.BufferedPaintDC(self, self.plotBuffer)
    ...
    def getCursorLocation(self):
        ...
        if minX < x < maxX and minY < y < maxY:
            return "plot"
        if minX < x < maxX and y > minY:
            return "xAxis"
        if minY < y < maxY and x < minX:
            return "yAxis"
        return "blank"
    ...
    def getPoint(self, xPos=None, coord="screen"):
        ...
        graphics = self.lastDraw[0]
        return graphics.getPoint(self.currentObject, xPos, coord)
    ...
    def drawLegend(self, dc, graphics):
        ...
        for name in names:
            ...
            pen = wx.Pen(pencolour, int(1 * self.printerScale["drawings"]), wx.SOLID)
    ...
    def drawCursorTracker(self):
        ...
        # If ERASING, simply restore the pristine background buffer!
        if self.mouseTracker:
            dc.DrawBitmap(self.plotBuffer, 0, 0, False)
            # Clear tracked text keys so drawInvertedText doesn't accumulate them
    ...
    def drawDistanceTracker(self):
        ...
        # draw tracker
        dc = wx.ClientDC(self)
        if getattr(self, "_distanceTrackerDrawn", False):
            dc.DrawBitmap(self.plotBuffer, 0, 0, False)
            if hasattr(self, "_drawnTextKeys"):
                self._drawnTextKeys.clear()
            self._distanceTrackerDrawn = False
            return
        self._distanceTrackerDrawn = True
        dc.SetLogicalFunction(wx.COPY)
        dc.SetPen(wx.Pen(wx.BLACK))
    ...
    def drawPointTracker(self):
        ...
        # draw tracker lines
        dc = wx.ClientDC(self)
        if getattr(self, "_pointTrackerDrawn", False):
            dc.DrawBitmap(self.plotBuffer, 0, 0, False)
            if hasattr(self, "_drawnTextKeys"):
                self._drawnTextKeys.clear()
            self._pointTrackerDrawn = False
            return

        self._pointTrackerDrawn = True
        dc.SetPen(wx.Pen(wx.BLACK))
        dc.SetLogicalFunction(wx.COPY)
        if wx.Platform == "__WXMAC__":
    ...
    def drawZoomBox(self):
        ...
        # set canvas and pen
        dc = wx.ClientDC(self)
        if getattr(self, "_zoomBoxDrawn", False):
            dc.DrawBitmap(self.plotBuffer, 0, 0, False)
            if hasattr(self, "_drawnTextKeys"):
                self._drawnTextKeys.clear()
            self._zoomBoxDrawn = False
            return
        self._zoomBoxDrawn = True
        dc.SetPen(wx.Pen(wx.BLACK))
    ...
    def drawSelectionRect(self):
        ...
        # draw tracker lines
        dc = wx.ClientDC(self)
        if getattr(self, "_selRectDrawn", False):
            dc.DrawBitmap(self.plotBuffer, 0, 0, False)
            if hasattr(self, "_drawnTextKeys"):
                self._drawnTextKeys.clear()
            self._selRectDrawn = False
            return
        self._selRectDrawn = True
        dc.SetLogicalFunction(wx.COPY)
        dc.SetPen(wx.Pen(wx.BLACK))
        dc.SetBrush(wx.Brush(wx.BLACK, wx.TRANSPARENT))
        dc.DrawRectangle(int(x1), int(y1), int(width), int(height))
    ...
    def drawSelectionRange(self):
        ...
        # draw tracker lines
        dc = wx.ClientDC(self)
        if getattr(self, "_selRangeDrawn", False):
            dc.DrawBitmap(self.plotBuffer, 0, 0, False)
            if hasattr(self, "_drawnTextKeys"):
                self._drawnTextKeys.clear()
            self._selRangeDrawn = False
            return
        self._selRangeDrawn = True
        dc.SetLogicalFunction(wx.COPY)
        dc.SetPen(wx.Pen(wx.BLACK))
        dc.SetBrush(wx.Brush(wx.BLACK, wx.TRANSPARENT))
        if wx.Platform == "__WXMAC__":
    ...
    def drawPointArrow(self, x, y, direction="up"):
        ...
        # draw arrow
        if direction == "up":
            dc.DrawPolygon([(int(x), int(y)), (int(x - 3), int(y + 7)), (int(x + 3), int(y + 7))])
        elif direction == "down":
            dc.DrawPolygon([(int(x), int(y)), (int(x - 3), int(y - 7)), (int(x + 3), int(y - 7))])
        elif direction == "left":
            dc.DrawPolygon([(int(x + 7), int(y + 2)), (int(x), int(y + 5)), (int(x + 7), int(y + 8))])
    ...
    def drawInvertedText(self, dc, text, x, y, font):
        ...
        # draw under others
            ...
            if key in self._drawnTextKeys:
                # We are ERASING!
                self._drawnTextKeys.remove(key)
                memDC = wx.MemoryDC()
                memDC.SelectObject(self.plotBuffer)
                dc.Blit(int(x), int(y), int(size[0]), int(size[1]), memDC, int(x), int(y), wx.COPY)
                memDC.SelectObject(wx.NullBitmap)
            else:
                # We are DRAWING!
                self._drawnTextKeys.append(key)
                dc.SetLogicalFunction(wx.COPY)
...
def _scaleFont(font, scale):
    ...
    # make print font
    return wx.Font(pointSize, family, style, weight, underline, faceName, encoding)
