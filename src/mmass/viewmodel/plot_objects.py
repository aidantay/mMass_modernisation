...
class Annotations:
    ...
    def draw(self, dc, printerScale) -> None:
        ...
        if self.properties["showPoints"]:
            ...
            pen = wx.Pen(pencolour, int(1 * printerScale["drawings"]), wx.SOLID)
...
class Points:
    ...
    def draw(self, dc, printerScale) -> None:
        ...
        if self.properties["showLines"] and len(self.scaled) > 1:
            ...
            dc.SetPen(pen)
            dc.SetBrush(brush)
            dc.DrawLines([(int(p[0]), int(p[1])) for p in self.scaled])
...
class Spectrum:
    ...
    def makeLabels(self, dc, printerScale):
        ...
        for x, peak in enumerate(self.peaklistScaled):
            ...
            if self.properties["labelGroup"] and self.peaklistCroppedPeaks[x].group:
                label += f" [{self.peaklistCroppedPeaks[x].group}]"
    ...
    def _drawSpectrum(self, dc, printerScale) -> None:
        ...
        # draw lines
        dc.DrawLines([(int(p[0]), int(p[1])) for p in self.spectrumScaled])
        ...
        for point in self.spectrumScaled:
            ...
            if xPos - lastX >= printerScale["drawings"]:
                ...
                with contextlib.suppress(BaseException):
                    dc.DrawRectangle(
                        int(lastX), int(gelY1), int(xPos - lastX), int(gelHeight)
                    )
    ...
    def _drawPeaklist(self, dc, printerScale) -> None:
        ...
        for x, peak in enumerate(self.peaklistScaled):
            if self.peaklistCroppedPeaks[x].isotope != 0:
                try:
                    dc.DrawLine(int(peak[0]), int(peak[2]), int(peak[0]), int(peak[1]))
                    dc.DrawLine(
        ...
        for x, peak in enumerate(self.peaklistScaled):
            if self.peaklistCroppedPeaks[x].childScanNumber is not None:
                with contextlib.suppress(OverflowError):
                    dc.DrawCircle(
                        int(peak[0]), int(peak[1]), int(3 * printerScale["drawings"])
                    )
    ...
    def _drawGelLegend(self, dc, gelCoords, gelHeight, printerScale) -> None:
        ...
        y = gelY1 + (gelHeight) / 2
        dc.DrawCircle(int(x), int(y), int(3 * printerScale["drawings"]))
    ...
    def _normalization(self) -> None:
        ...
        if len(self.spectrumPoints) and len(self.peaklistPoints):
            _spectrumMinXY, spectrumMaxXY = self.spectrumBox
            _peaklistMinXY, peaklistMaxXY = self.peaklistBox
