...
def saveEnzymes(path=os.path.join(blocksdir, "enzymes.xml")):
    ...
    names.sort()
    for name in names:
        buff += f'  <enzyme name="{_escape(enzymes[name].name)}">\n'
        buff += f"    <expression><![CDATA[{enzymes[name].expression}]]></expression>\n"
        buff += f'    <formula nTerm="{enzymes[name].nTermFormula}" cTerm="{enzymes[name].cTermFormula}" />\n'
        buff += f'    <allowMods before="{int(enzymes[name].modsBefore)}" after="{int(enzymes[name].modsAfter)}" />\n'
        buff += "  </enzyme>\n"
...
def saveModifications(path=os.path.join(blocksdir, "modifications.xml")):
    ...
    names.sort()
    for name in names:
        buff += f'  <modification name="{_escape(modifications[name].name)}">\n'
        buff += f"    <description>{_escape(modifications[name].description)}</description>\n"
        buff += f'    <formula gain="{modifications[name].gainFormula}" loss="{modifications[name].lossFormula}" />\n'
        buff += f'    <specifity amino="{modifications[name].aminoSpecifity}" terminus="{modifications[name].termSpecifity}" />\n'
        buff += "  </modification>\n"
