...
class panelCompoundsSearch(wx.MiniFrame):
    ...
    def onListCopy(self, evt: wx.Event = None) -> None:
        ...
        for item in self.currentCompounds:
            for annotation in item[-1]:
                ...
                if item[3] == "radical":
                    ...
                    annotation.label += f" ({item[3]} adduct)"
    ...
    def updateCompoundsList(self) -> None:
        ...
        for index, item in enumerate(self.currentCompounds):
            # filter data
            if (self._compoundsFilter == 1 and item[5] is None) or (self._compoundsFilter == -1 and item[5] is not None):
                continue
                ...
                for z in range(1, maxCharge):
                    ...
                    for item in config.compoundsSearch["adducts"]:
                        if item in ("Na", "K", "Li", "NH4", "ACN", "MeOH"):
                            formula = f"{compound.expression}({adducts[item]})(H-1)"
                        elif item in ("-H2O"):
                            ...
                            for item2 in ("ACN", "MeOH", "-H2O"):
                                if item2 in config.compoundsSearch["adducts"]:
                                    if item2 in ("ACN", "MeOH"):
                                        ...
                                        formula = f"{compound.expression}({adducts[item1]})({adducts[item2]})(H-2)"
                                    elif item2 in ("-H2O"):
                                        adduct = f"{item1}{item2}"
    ...
    def calibrateByMatches(self, references: list) -> None:
